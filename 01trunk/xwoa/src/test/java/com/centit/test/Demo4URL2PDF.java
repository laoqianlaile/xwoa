...
public class Demo4URL2PDF {
    ...
    private static String formatContentTag(org.jsoup.nodes.Element entry) {
        try {
            ...
            // 把 &lt;a href="*.jpg" &gt;&lt;img src="*.jpg"/&gt;&lt;/a&gt; 替换为
            // &lt;img
            // src="*.jpg"/&gt;
     ...
     * 
     * FileOutputStream outputStream = new FileOutputStream(pdfFile); PdfWriter
     * pdfwriter = PdfWriter.getInstance(document, outputStream); //
     * pdfwriter.setViewerPreferences(PdfWriter.HideToolbar); document.open();
     * 
     * BaseFont bfCN = BaseFont.createFont("STSongStd-Light", "UniGB-UCS2-H",
     * false); // 中文字体定义 Font chFont = new Font(bfCN, 14, Font.NORMAL,
     * BaseColor.BLUE); Font secFont = new Font(bfCN, 12, Font.NORMAL, new
     * BaseColor(0, 204, 255)); Font textFont = new Font(bfCN, 12, Font.NORMAL,
     * BaseColor.BLACK);
     * 
     * int chNum = 1; Chapter chapter = new Chapter(new
     * Paragraph("URL转PDF元素，便于追加其他内容", chFont), chNum++);
     * 
     * String[] blogInfo = extractBlogInfo(blogURL);
     * 
     ...
     * secFont));
     * 
     * section.setIndentation(10); section.setIndentationLeft(10);
     * section.setBookmarkOpen(false);
     * section.setNumberStyle(Section.NUMBERSTYLE_DOTTED_WITHOUT_FINAL_DOT);
     * section.add(new Chunk("分类：" + blogInfo[1] + " 发表日期：" + blogInfo[2],
     * textFont)); LineSeparator line = new LineSeparator(1, 100, new
     * BaseColor(204, 204, 204), Element.ALIGN_CENTER, -2); Paragraph p_line =
     * new Paragraph(); p_line.add(line); section.add(p_line);
     * section.add(Chunk.NEWLINE);
     * 
     * final List<Element> pdfeleList = new ArrayList<Element>(); ElementHandler
     * elemH = new ElementHandler() {
     * 
     * public void add(final Writable w) { if (w instanceof WritableElement) {
     * pdfeleList.addAll(((WritableElement) w).elements()); }
     * 
     * }
     * 
     * @Override public void onStart(ElementPath elementPath) { // TODO
     * Auto-generated method stub
     * 
     * }
     * 
     * @Override public void onEnd(ElementPath elementPath) { // TODO
     * Auto-generated method stub
     * 
     * } }; XMLWorkerHelper.getInstance().parseXHtml(elemH, new
     * InputStreamReader(parse2Stream(blogInfo[3]), "utf-8"));
     * 
     * List<Element> list = new ArrayList<Element>(); for (Element ele :
     * pdfeleList) { if (ele instanceof LineSeparator || ele instanceof
     * WritableDirectElement) { continue; } list.add(ele); }
     * section.addAll(list);
     * 
     ...
     * 
     * document.add(chapter); document.close(); } catch (Exception e) {
     * e.printStackTrace(); }
     * 
     * }
     */
