...
(function(jq){
  jq.fn.jqTreeTable=function(map, options){
    ...
    mapa=[],mapb=[],tid=this.attr("id"),collarr=[],
	  stripe=function(){
      if(opts.striped){
  		  $("#"+tid+" tr:visible").filter(":even").addClass("even").end().filter(":odd").removeClass("even").addClass("odd");
      }
	  },
    ...
    jq(".parimg", this).each(function(i){
      ...
      jqt.click(function(){
        ...
        /*var $tr = $("#treet2 tr").not(".collapsed"),
		len = $tr.length;
	    for(var i=0;i<len;i++){
			if(i%2==0)$($tr[i]).attr("class","").addClass("even");
			else $($tr[i]).attr("class","").addClass("odd");
		}*/
      });
    });
