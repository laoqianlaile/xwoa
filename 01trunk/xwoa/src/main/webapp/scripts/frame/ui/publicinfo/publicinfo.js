define(['jquery', 
        ...
        'Msg', 
        'Core',
        'text!ui/publicinfo/template/publicinfo-file-line.xml',
        'text!ui/publicinfo/template/publicinfo-new-folder.xml',
        'Upload'], 
        function($, EventManager, Msg, Core, TEMPLATE_FILE, TEMPLATE_NEWFOLDER) { 
	...
	function getFileURL(file) {
		// 文件夹
		if (file.isfolder=='1') {
			return $.publicinfo.FILE_OPTION.public_url+'?random='+Math.random()+'&path='+file.infocode;
		}
	...
	function getSort(info) {
	 ...
	 function createPath(data) {
	 	...
	 	if (parentcode && parentcode != '0') {
	 		$('a.uplevel', line).attr('href', $.publicinfo.FILE_OPTION.public_url+'?random='+Math.random()+'&path='+parentcode).show();
	 	}
	 	else {
	 		$('a.uplevel', line).hide();
	 	}
	 	...
	 	if (infocode == unitroot) {
	 		...
	 		$('a.home', line).attr('href', $.publicinfo.FILE_OPTION.public_url+'?random='+Math.random()+'&path='+unitroot).show();
	 	}
	 	...
	 	if (data.path) {
	 		...
	 		for (var i=0; i<length; i++) {
	 			if (i == length - 1) {
	 				...
	 				$('div.ed', line).append('&nbsp;&nbsp;<a class="patha" type="dir" href="' + $.publicinfo.FILE_OPTION.public_url+'?random='+Math.random()+'&path='+path[i].INFOCODE+'">'+ path[i].FILENAME +'</a>&nbsp;&nbsp;&gt;');
	 			}
	 		}
	 	}
	...
	function getContextPath() {
	 ...
	 function initUploadButton() {
		 ...
		 if($(".publicinfo-upload-add")){
			 if((($.publicinfo.FILE_OPTION.authority >> $.publicinfo.FILE_OPTION.AUTH_ADD) % 2) == 1 ){
				 $(".publicinfo-upload-add").show();
			 }else{
				 $(".publicinfo-upload-add").hide();
			 }
           }
	 ...
	 function initModifyButton() {
		 ...
		 if($(".publicinfo-upload-delect")||$(".publicinfo-upload-modify")){
			 for (var i=0; i<$.publicinfo.SELECTED_FILES.length; i++) {
					...
					if (!$.publicinfo.authenticate(file.data('authority'), $.publicinfo.FILE_OPTION.AUTH_MODIFY)) {
						 $(".publicinfo-upload-delect").hide();
						 $(".publicinfo-upload-modify").hide();
						 break;
					}
					 $(".publicinfo-upload-delect").show();
					 $(".publicinfo-upload-modify").show();
				}
           }
	   }
	...
	$.publicinfo = {
			/*公共文件设置*/
			FILE_OPTION : {
				public_url: getContextPath()+'/app/publicinfo!dirPublicFolder.do?random='+Math.random(),	// 查询公共文件夹
				personal_url: getContextPath()+'/app/publicinfo!dirPersonalFolder.do',	// 查询个人文件夹
             	view_url: getContextPath()+'/app/publicinfo!view.do',						// 查看文件
				add_url: getContextPath()+'/app/publicinfo!addFolder.do?random='+Math.random(),					// 添加文件夹
				download_url: getContextPath()+'/app/publicinfo!download.do',		// 下载文件（公共文件夹处理）
				toDownload_file_url: getContextPath()+'/app/fileinfoFs!toDownloadfile.do',	// 准备下载文件（文件处理）
				download_file_url: getContextPath()+'/app/fileinfoFs!downloadfile.do',		// 下载文件（文件处理）
				delete_url: getContextPath()+'/app/publicinfo!delete.do?random='+Math.random(),					// 删除文件
				rename_url: getContextPath()+'/app/publicinfo!rename.do',					// 重命名文件
				copy_url: getContextPath()+'/app/publicinfo!copy.do',						// 复制文件
				move_url: getContextPath()+'/app/publicinfo!move.do',						// 移动文件
				upload_url: getContextPath()+'/app/publicinfo!upload.do;jsessionid=${pageContext.session.id}',					// 上传文件
				search_url: getContextPath()+'/app/searcher!retrieve.do',
			 ...
			 * 初始化
			 */
			init: function(mode,urlObj,rootunitcode) {
				
				if(urlObj){
					$.extend($.publicinfo.FILE_OPTION,urlObj);
				}
				$.publicinfo.MODE = mode;
				$.publicinfo.UNITCODE = rootunitcode;//部门编号;
				
				
				var path = $('#path').val();
				
				// 加载文件夹
				this.dirPublicFolder(path);
				...
				publicinfoManager.addEvent("dirFolder")
					...
					.setEventAction("excute", function(event, obj) {
						...
						$.getJSON(obj.href, {mode: $.publicinfo.MODE, rootunitcode: $.publicinfo.UNITCODE}, function(data) {
				...
				publicinfoManager.addEvent("rename")
					...
					// 打开重命名控件
					.addEventLitsen("prepare", ".line.operation a.more", "click")
					.setEventAction("prepare", function(event, obj, result) {
				...
				publicinfoManager.addEvent("downloadTemp")
					...
					.setEventAction("excute", function(event, obj) {
						...
						$.ajax({
							url:$.publicinfo.FILE_OPTION.download_url,
							type:"post",
							data:{"infocode": code, "mode": $.publicinfo.MODE ,rootunitcode: $.publicinfo.UNITCODE },
							dataType:"json",
							async:false ,
			...
			validateCopyMoveFile: function(nodes, type) {
				...
				if (0 != $.publicinfo.SELECTED_FILES.length && flag) {
					Msg.info("有部分选择项目无权限移动，仅会操作可移动项目。");
				}
			...
			copyMoveFile: function(event, obj, result) {
				...
				if (type == 'copy') {
					$.post($.publicinfo.FILE_OPTION.copy_url, {infocode: files.join(','), toInfocode: node.id, mode: $.publicinfo.MODE ,rootunitcode: $.publicinfo.UNITCODE }, function(data) {
						eventObject.excuteAction("callback", data);
					}, 'json');
				}
				else if (type == 'move') {
					$.post($.publicinfo.FILE_OPTION.move_url, {infocode: files.join(','), toInfocode: node.id, mode: $.publicinfo.MODE ,rootunitcode: $.publicinfo.UNITCODE }, function(data) {
						eventObject.excuteAction("callback", data);
					}, 'json');
				}
			...
			copyMoveFileCallback: function(data, result) {
				...
				if (file.infocode == $.publicinfo.FILE_OPTION.infocode) {
					Msg.info(typeName + '文件成功');
					$.publicinfo.dirPublicFolder();
					$.publicinfo.COPY_MOVE_WINDOW.modal('hide');
					return;
				}
				
				Msg.confirm(typeName + '文件成功，是否跳转到 '+file.filename+' 去查看？', {
			...
			deleteFile: function() {
				...
				Msg.confirm(alertMsg, {
					okCall: function() {
						$.post($.publicinfo.FILE_OPTION.delete_url, {infocode: files.join(','), mode: $.publicinfo.MODE ,rootunitcode: $.publicinfo.UNITCODE}, function(data) {
							eventObject.excuteAction("callback", data);
						}, 'json');
					}
				});
			...
			downloadFile: function(event) {
				...
				$.ajax({
					url:$.publicinfo.FILE_OPTION.download_url,
					type:"post",
					data:{"infocode": files.join(','), "mode": $.publicinfo.MODE ,rootunitcode: $.publicinfo.UNITCODE },
					dataType:"json",
					async:false ,
					success:function(data){
						eventObject.excuteAction("callback", data);	
			...
			downloadFileCallback: function(data, result) {
				...
				$.ajax({
					async:false,
					url:$.publicinfo.FILE_OPTION.toDownload_file_url,
					type:"post",
					dataType:"json",
					data:{
			...
			rename: function() {
				...
				$.post($.publicinfo.FILE_OPTION.rename_url, {mode:$.publicinfo.MODE,rootunitcode: $.publicinfo.UNITCODE, name:newName, infocode:file.data('infocode'), root:$.publicinfo.FILE_OPTION.infocode}, function(data){
					eventObject.excuteAction("callback", data);
				}, 'json');
			},
			...
			addFolder: function(event, obj, result) {
				if (result === true) {
					...
					$.post($.publicinfo.FILE_OPTION.add_url, {filename:input.val(), root:$.publicinfo.FILE_OPTION.infocode, mode: $.publicinfo.MODE ,rootunitcode: $.publicinfo.UNITCODE }, 
							function(data) {
						eventObject.excuteAction("callback", data);
					}, 'json');
				};
			...
			dirPublicFolder: function(toPath) {
				...
				var path = toPath || $.publicinfo.FILE_OPTION.infocode;
				 ...
				 if (path ) {
					$.getJSON($.publicinfo.FILE_OPTION.public_url+'?random='+Math.random(), {path:path, mode: $.publicinfo.MODE ,rootunitcode: $.publicinfo.UNITCODE  }, $.publicinfo.dirPublicFolderCallback);
				}
				else {
					$.getJSON($.publicinfo.FILE_OPTION.public_url+'?random='+Math.random(), {mode: $.publicinfo.MODE  ,rootunitcode: $.publicinfo.UNITCODE }, $.publicinfo.dirPublicFolderCallback);
				}
			},
			...
			dirPublicFolderCallback: function(data, result){
				...
				if (data && data.result != 0) {
					Msg.error('错误[' + data.result + '] ' + data.description);
					return false;
				}
...
//				$.publicinfo.FILE_OPTION.rootunitcode = data.unitcode;
				...
				var isOneself = data.isOneself;
				if('Y' == isOneself && 'PERSONFILE' == $.publicinfo.MODE){
					$('.line div.owner').html('&nbsp;');
			...
			addFile: function(file, last, container) {
				...
				var subFilename = file.filename.length > 20 ? file.filename.substring(0,20) + '...' : file.filename;
				
				var html = TEMPLATE_FILE
				.replaceAll('{infocode}', file.infocode)
				// 替换类型
				.replaceAll('{type}', getFileType(file))
				.replaceAll('{fullFilename}', getFullFilename(file))
				// 替换文件名
				.replaceAll('{filename}', subFilename)
				// 替换类型
				...
				//不能直接访问的文件类型不显示链接
				if('none'==getLinkType(file)){
					html=html.replaceAll("<a.*?[^<]+</a>" ,subFilename);
				}
