$(function() {
	...
	$.myUpload = {
			...
			afterUploadSuccess: function(data) {
				...
				if (window.DWZ) {
					container.append('<tr id="file_'+filecode+'" > <td>'+filename+'</td> <td>'+filesize+'</td> <td><a href="'+window.DWZ.contextPath+'/app/fileinfoFs!deletefile.do?filecode='+filecode+'" class="delete" inputID="'+this.settings.inputID+'" filecode="'+filecode+'">删除</a></td><tr>');
				}
				else {
					container.append('<tr id="file_'+filecode+'" > <td>'+filename+'</td> <td>'+filesize+'</td> <td><a href="'+window.Config.contextPath+'/app/fileinfoFs!deletefile.do?filecode='+filecode+'" class="delete" inputID="'+this.settings.inputID+'" filecode="'+filecode+'">删除</a></td><tr>');
				}
	...
	$.myUpload.options = {
		...
		// 必选设置
		swf: window.DWZ ? window.DWZ.contextPath+'/scripts/uploadify-v3.2.1/uploadify.swf' : window.Config.contextPath+'/scripts/uploadify-v3.2.1/uploadify.swf',
		uploader: window.DWZ ? window.DWZ.contextPath+'/app/fileinfoFs!uploadfile.do' : window.Config.contextPath+'/app/fileinfoFs!uploadfile.do'
	};
});