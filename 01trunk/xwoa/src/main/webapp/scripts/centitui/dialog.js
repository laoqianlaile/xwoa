/**
 * @author Roger Wu
 * reference:CentitUI.drag.js, CentitUI.dialogDrag.js, CentitUI.resize.js, CentitUI.taskBar.js
 */
(function($){
	$.pdialog = {
		...
		open:function(url, dlgid, title, options) {
			...
			if(dialog) {
				...
				$("body").append(CentitUI.frag["dialogFrag"]);
				...
				// ADD @zk 添加触发源和close事件
				if(options.target) dialog.data('target', options.target);
				...
				if(op.drawable){
					...
					moveTip(document.getElementById("pw_dialog"),document.getElementById("pw_dialogHeader"),document.getElementById("pw_shadow"));				
				}
...
//					$.pdialog.switchDialog(dialog);
		...
		close:function(dialog) {
			...
			if(close && $.isFunction(close)) {
				...
				if(param && param != ""){
					param = CentitUI.jsonEval(param);
					go = close.call(dialog, param);
				} else {
					go = close.call(dialog, param);
				}
				if(!go) return;
...
//				$.taskBar.closeDialog($(dialog).data("id"));
		...
		checkTimeout:function(){
			...
			var json = CentitUI.jsonEval($conetnt.html());
			if (json && json.statusCode == CentitUI.statusCode.timeout) this.closeCurrent();
		},
