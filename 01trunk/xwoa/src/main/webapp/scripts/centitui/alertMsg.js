...
$.setRegional("alertMsg", {
	title:{error:"出错啦", info:"提示", warn:"警告", correct:"成功", confirm:"确认"},
	butMsg:{ok:"是", yes:"是", no:"否", cancel:"取消"}
});
var alertMsg = {
	_boxId: "#alertMsgBox",
	...
	_open: function(type, msg, buttons){
		...
		if (buttons) {
			for (var i = 0; i < buttons.length; i++) {
				...
				butsHtml += CentitUI.frag["alertButFrag"].replace("#butMsg#", buttons[i].name).replace("#callback#", sRel);
			}
		}
		var boxHtml = CentitUI.frag["alertBoxFrag"].replace("#type#", type).replace("#title#", this._getTitle(type)).replace("#message#", msg).replace("#butFragment#", butsHtml);
