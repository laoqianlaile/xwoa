...
var CentitUI = {
	...
	loadLogin:function(){
		if ($.pdialog && CentitUI._set.loginTitle) {
			$.pdialog.open(CentitUI._set.loginUrl, "login", CentitUI._set.loginTitle, {mask:true,width:520,height:260});
		} else {
			window.location = CentitUI._set.loginUrl;
		}
	},
	...
	ajaxDone:function(json){
		...
		if(json.statusCode == CentitUI.statusCode.error) {
			if(json.message && alertMsg) alertMsg.error(json.message);
		} else if (json.statusCode == CentitUI.statusCode.timeout) {
			if(alertMsg) alertMsg.error(json.message || CentitUI.msg("sessionTimout"), {okCall:CentitUI.loadLogin});
			else CentitUI.loadLogin();
		} else {
	...
	init:function(pageFrag, options){
		...
		$.extend(CentitUI.statusCode, op.statusCode);
		$.extend(CentitUI.pageInfo, op.pageInfo);
		...
		jQuery.ajax({
			...
			success: function(xml){
				$(xml).find("_PAGE_").each(function(){
					...
					if (pageId) CentitUI.frag[pageId] = $(this).text();
				});
				
				$(xml).find("_MSG_").each(function(){
					var id = $(this).attr("id");
					if (id) CentitUI._msg[id] = $(this).text();
				});
	...
	$.fn.extend({
		...
		ajaxUrl: function(op){
			...
			$.ajax({
				...
				success: function(response){
					var json = CentitUI.jsonEval(response);
					CentitUI.debug("statusCode:"+json.statusCode);
					if (json.statusCode==CentitUI.statusCode.timeout){
						alertMsg.error(json.message || CentitUI.msg("sessionTimout"), {okCall:function(){
		...
		layoutH: function($refBox){
			return this.each(function(){
				...
				var parent = $this.parents("div.unitBox:first");
				...
				if ($this.isTag("table")) {
					$this.removeAttr("layoutH").wrap('<div layoutH="'+iLayoutH+'" style="width:100%;overflow:auto;height:'+iH+'px"></div>');
				} else {
					$this.height(iH).css("overflow","auto");
				}
			});
		},
		layoutW: function($refBox){
			return this.each(function(){
				var $this = $(this);
				if (! $refBox) $refBox = $this.parents("div.layoutBox:first");
				var iRefW = $refBox.width();
				var iLayoutW = parseInt($this.attr("layoutW"));
				var iW = iRefW - iLayoutW > 50 ? iRefW - iLayoutW : 50;
				
				$this.height(iW).css("overflow","auto");
			});
		},
...
//       }catch (ex){}
	...
	if (ifm != null && subWeb != null)
		ifm.height = subWeb.body.scrollHeight + 35;
//	if ($.browser.msie)
