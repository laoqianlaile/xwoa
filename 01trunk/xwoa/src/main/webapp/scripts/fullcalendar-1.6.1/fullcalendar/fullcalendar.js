...
var defaults = {
	...
	//后期添加begin
	noneTitle : false,//显示内容较小时，不显示title，鼠标移上去时显示
...
function setMinHeight(element, height) {
	...
	element.each(function(i, _element) {
        ...
        //alert(_element.style.cssText.toString());
	});
}
...
function AgendaEventRenderer() {
	...
	function slotSegHtml(event, seg) {
		...
		var default_fc_event = 'fc-event';
		if(null != event.tasktag && '' != event.tasktag) {
			default_fc_event = default_fc_event + '-' + event.tasktag;
		}
		var classes = [default_fc_event, 'fc-event-vert'];
		if (isEventDraggable(event)) {
...
function DayEventRenderer() {
	...
	function daySegHTML(segs) { // also sets seg.left and seg.outerWidth
		...
		for (i=0; i<segCnt; i++) {
			...
			classes = [default_fc_event, 'fc-event-hori'];
			if (isEventDraggable(event)) {
				classes.push('fc-event-draggable');
			}
			if (seg.isStart) {
			...
			if(!opt('noneTitle')) {
				...
				if (!event.allDay && seg.isStart) {
					html +=
						"<span class='fc-event-time' title='" + htmlEscape(event.title) + "'>" +
						htmlEscape(formatDates(event.start, event.end, opt('timeFormat'))) +
						"</span>";
				}
				html +=
