...
HROS.appmanage = (function(){
	return {
		...
		*/
		...
		showFolder: function() {
			$('#amg_folder_container li[type=folder]').hover(function(e) {
				var obj = $(this);
				window.HANDLE_OPEN_FOLDERVIEW = setTimeout(function() {
					// 如果没有打开本文件夹浏览
					if (!$('qv_' + obj.attr('appid'))[0]) {
						HROS.folderView.get(obj);
					}
				}, 300);
				
			}, function(e) {
				var toElement = $(e.toElement).hasClass('quick_view_container') ? $(e.toElement) : $(e.toElement).parents('div.quick_view_container');
				
				// 鼠标移动到文件夹浏览区域
				if (toElement.hasClass('quick_view_container')){
					toElement.mouseleave(function(e) {
						var toElement = $(e.toElement).is('li.appbtn') ? $(e.toElement) : $(e.toElement).parents('li.appbtn');
						
						// 鼠标没有移回文件列表
						if (!$(e.toElement).is('li.appbtn')) {
							HROS.folderView.hide();
						}
					});
				}
				else {
					clearTimeout(window.HANDLE_OPEN_FOLDERVIEW);
					HROS.folderView.hide();
				}
			});
		}
