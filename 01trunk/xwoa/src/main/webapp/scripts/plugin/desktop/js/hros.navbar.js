		...
		*/
		getAvatar : function(){
			$.ajax({
				...
				$('#nav-bar .indicator-header-img').attr('src', HROS.CONFIG.contextPath + msg);
			});
		},
		 ...
		 */
		changePwd : function(){
			$('#navbarHeaderImg, img.indicator-header-img').unbind('click');
			$('#navbarHeaderImg, img.indicator-header-img').click(function(){
				HROS.window.createTemp({appid : 'appid_change_pwd', title:"修改密码",url:HROS.CONFIG.contextPath +"/sys/userDef!modifyPwdPage.do",width:500,height:400,isresize:false,isopenmax:false,isflash:false});
			});
		},
