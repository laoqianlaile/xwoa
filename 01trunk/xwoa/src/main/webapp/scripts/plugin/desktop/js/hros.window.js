		...
		createTemp : function(obj){
			...
			if(!iswindowopen){
				...
				nextDo({
					...
					appid : appid,
					realappid : appid,
					imgsrc : HROS.CONFIG.contextPath + '/scripts/plugin/desktop/img/ui/default_icon.png',
					title : obj.title,
					url : obj.url,
					width : obj.width,
					height : obj.height,
		...
		*/
					...
					switch(options.type){
						...
						case 'papp':
							...
							TEMP.windowTemp = {
								...
								'realappid' : options.realappid,
								'title' : options.title,
								'url' : /^http/.test(options.url) ? options.url : HROS.CONFIG.contextPath + options.url,
								'imgsrc' : options.imgsrc,
		...
		handle : function(){
			$('#desk').on('dblclick', '.window-container .title-bar', function(e){
				...
				var btn = $(this);
				var iframe = btn.data('iframe');
				if (!iframe) {
					iframe = obj.find('iframe');
					btn.data('iframe', iframe);
				}
				$.history.clear(iframe);
				
				HROS.window.refresh(obj.attr('appid'));
			}).on('click', '.window-container .goback', function(){
				var btn = $(this);
				var iframe = btn.data('iframe');
				
				if (!iframe) {
					iframe = btn.closest('div.window-container').find('iframe');
					btn.data('iframe', iframe);
				}
				...
				$('.quick_view_container').remove();
				return false;
			}).on('click', 'a.indicator-manage', function() {
				HROS.appmanage.set();
			})/*.on('mouseenter', 'a.indicator-manage', function() {
				window.HANDLE_ALLOVERVIEW = setTimeout(function() {
					HROS.appmanage.set();
				}, 1050);
			}).on('mouseleave', 'a.indicator-manage', function() {
				clearTimeout(window.HANDLE_ALLOVERVIEW);
			})*/;
		},
		move : function(){
