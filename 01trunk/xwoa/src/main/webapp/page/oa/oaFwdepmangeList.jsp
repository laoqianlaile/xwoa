	...
	<script type="text/javascript">
		...
		$(function() {
			...
			var param = {
				...
				callback : {
					onClick : function(eventjs, treeId, treeNode) {
                           $("#tabFrames1").attr("src",'<%=request.getContextPath()%>/dispatchdoc/oaFwdepmange!mangeDetail.do?s_unitcode='+ treeNode.id);
					  }
					}
			...
			if (nodes.length>0) {
				...
				$("#tabFrames1").attr("src",'<%=request.getContextPath()%>/dispatchdoc/oaFwdepmange!mangeDetail.do?s_unitcode='+ nodes[0].id+ '&s_queryUnderUnit=true');
				
				debugger;
				var srcNode = treeObj.getSelectedNodes();
			    var isNode = srcNode[0];
				if(!isNode.isParent){
		             $("#innermsg_td").css("width", "200px");
				   }
