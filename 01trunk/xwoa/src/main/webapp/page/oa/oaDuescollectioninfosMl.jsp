...
<body>
	<table cellpadding="0" cellspacing="0" align="left"
		...
		<tr>
			<td id="innermsg_td" width="15%" style="position: relative; background: #f0f6e4;vertical-align:top;">
	...
	<script type="text/javascript">
		...
		$(function() {
			...
			var param = {
				...
				callback : {
					onClick : function(eventjs, treeId, treeNode) {
                           $("#tabFrames1").attr("src",'<%=request.getContextPath()%>/oa/oaDuescollection!mangeDetail.do?s_djId=${djId}'+'&s_unitcode='+ treeNode.id);
					  }
					}
			...
			innermsg.funs.init($.parseJSON('${units}'));
			
			var treeObj =innermsg.funs.getTree();
			var nodes = treeObj.getNodes();
			if (nodes.length>0) {
				treeObj.selectNode(nodes[0]);
				$("#tabFrames1").attr("src",'<%=request.getContextPath()%>/oa/oaDuescollection!mangeDetail.do?s_djId=${djId}'+'&s_unitcode='+ nodes[0].id+ '&s_queryUnderUnit=true');
				
				debugger;
				var srcNode = treeObj.getSelectedNodes();
			    var isNode = srcNode[0];
				if(!isNode.isParent){
		             $("#innermsg_td").css("width", "0px");
				   }
