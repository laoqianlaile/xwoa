...
<ul id="infoTab">
	<li url="<%=request.getContextPath()%>/oa/oaCostInfo!list.do?s_djid=${object.djid}&djid=${object.djid }&show_type=${show_type}" <c:if test="${empty object.djid}">class="disable"</c:if>>费用明细</li>
	<li url="<%=request.getContextPath()%>/oa/oaAccidentRecord!list.do?s_djid=${object.djid}&djid=${object.djid }&show_type=${show_type}" <c:if test="${empty object.djid}">class="disable"</c:if>>事故记录</li>
	<li url="<%=request.getContextPath()%>/oa/oaTrafficRecord!list.do?s_djid=${object.djid}&djid=${object.djid }&show_type=${show_type}" <c:if test="${empty object.djid}">class="disable"</c:if>>违章记录</li>
	
</ul>
<div id="infoView">
    <!-- 修改id，不进入系统的comm*.js,没有iframe style -->
    <c:if test="${empty object.djid}">
    <iframe id="tabFrames1" name="tabFrames" src="<%=request.getContextPath()%>/oa/oaCostInfo!list.do?s_djid=NULL" onload="autoHeight(this);"  width="100%"
			frameborder="0" scrolling="auto" border="0" marginwidth="0"></iframe> 
    </c:if>
	    <c:if test="${not empty object.djid}">
    <iframe id="tabFrames1" name="tabFrames" src="<%=request.getContextPath()%>/oa/oaCostInfo!list.do?s_djid=${object.djid}&show_type=${show_type}" onload="autoHeight(this);"  width="100%"
			frameborder="0" scrolling="auto" border="0" marginwidth="0"></iframe> 
    </c:if>
