...
<s:submit name="save"  method="save" onclick="return checkForm();" cssClass="btn" key="opt.btn.submit"/>
    ...
    <input type="hidden" id="no" name="no" value="${object.no }"/>
     <input type="hidden" id="belongUnitcode" name="belongUnitcode" value="${object.belongUnitcode}"/>
 ...
 <tr>
				...
				<tr>
				  ...
				  <fmt:formatDate value='${object.filingdate}' pattern='yyyy-MM-dd HH:mm' />
					...
					<td align="left">
						<select id="filingannual" style="width:200px;height:25px;" name="filingannual" onchange="getTitanic();">
...
<script type="text/javascript">
   ...
   function checkForm(){
	   ...
	   if($("#titanic").val()!=''){
			var isvalid= (new RegExp(r)).test($("#titanic").val());
			if(!isvalid){
				alert("输入的件号格式不正确");
				$("#titanic").focus();
				return  false;
	   ...
	   //验证件号是否重复
	   var result = $.ajax({
			url: "${contextPath}/oa/oaArchive!checkTitanic.do?id=${id}&titanic=" + $("#titanic").val()+"&filingannual="+$("#filingannual").val()+"&belongUnitcode"+$("#belongUnitcode").val(),
			async: false
		}).responseText;
		if ("success" == result) {
			return true;
   ...
   function getTitanic(type, newLsh) {
	canSubmit = false;
	
	var url ="${contextPath}/oa/oaArchive!afterChange.do?id=${id}&titanic=" + $("#titanic").val()+"&filingannual="+$("#filingannual").val()+"&belongUnitcode="+$("#belongUnitcode").val();
	...
	$.ajax({
		...
		success : function(data) {
			if ("success" == data.status) {
				...
				$("#titanic").val(data.titanic);
			
			} else if ("unavailable" == data.status) {
				alert("获取新的文号发生错误");
			} else if ("" == $("#filingannual").val()) {
				alert("请先选择归档年份。"); 
			} 
