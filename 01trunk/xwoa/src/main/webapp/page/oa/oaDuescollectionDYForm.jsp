	...
	<script type="text/javascript">
		...
		$(function() {
			...
			var param = {
				...
				callback : {
					onClick : function(eventjs, treeId, treeNode) {
                           $("#tabFrames1").attr("src",'<%=request.getContextPath()%>/oa/oaDuescollection!mangeDyDetail.do?s_unitcode='+ treeNode.id);
					  }
					}
			...
			if (nodes.length>0) {
				...
				$("#tabFrames1").attr("src",'<%=request.getContextPath()%>/oa/oaDuescollection!mangeDyDetail.do?s_unitcode='+ nodes[0].id+ '&s_queryUnderUnit=true');
