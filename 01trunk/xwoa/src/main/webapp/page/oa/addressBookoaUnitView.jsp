			...
			var param = {
				...
				callback : {
					onClick : function(eventjs, treeId, treeNode) {
                             if('1'==treeNode.id){
                            	 $("#tabFrames1").attr("src",'<%=request.getContextPath()%>/oa/oaUserinfo!oaList.do?s_type=C&bodycode='+ treeNode.id+ '&type=' + treeNode.p+ '&s_queryUnderUnit=true');
                             }else{
                            	 $("#tabFrames1").attr("src",'<%=request.getContextPath()%>/oa/oaUserinfo!oaList.do?s_type=C&bodycode='+ treeNode.id+ '&type=' + treeNode.p);
                             }
...
// 								+ treeNode.id + '&type=' + treeNode.p;
			...
			if (nodes.length>0) {
				...
				$("#tabFrames1").attr("src",'<%=request.getContextPath()%>/oa/oaUserinfo!oaList.do?s_type=C&bodycode='+ nodes[0].id+ '&type=' + nodes[0].p+ '&s_queryUnderUnit=true');
			}
