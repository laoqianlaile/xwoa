...
<body class="sub-flow">
		<fieldset style="padding:10px;"  class="form">
			<legend   style="width:auto;margin-bottom:10px;">用户机构编辑--
			<c:out value="${cp:MAPVALUE('usercode',userUnit.id.usercode)}" />:
...
<%-- 			<s:submit method="saveUserUnit" cssClass="btn" value="保存" /> --%>
			...
			<table cellpadding="0" cellspacing="0" class="viewTable">
				...
				<tr>
					...
					<td align="left" >
						<s:textfield name="userUnit.id.usercode" value="%{userUnit.id.usercode}" readonly="true" style="width:200px;height:30px;" theme="simple" /> 
					</td>
				</tr>
				<tr>
					<td class="addTd">
				...
				<tr>
					...
					<input type="text" id="orgName" name="orgName" value="${cp:MAPVALUE('unitcode',userUnit.id.unitcode)}" style="width:200px;height:30px;">
				...
				<tr>
					...
					<c:if test="${not empty userUnit.id.userstation }">
						...
						<select  id="userstation" name="userUnit.id.userstation" disabled="disabled" style="width:200px;height:30px;">
					...
					<c:if test="${empty userUnit.id.userstation }">
						<select  id="userstation" name="userUnit.id.userstation" style="width:200px;height:30px;">
						<option value="">---请选择---</option>
							<c:forEach var="row" items="${cp:LVB('StationType')}">
               					 <option value="<c:out value='${row.value}'/>" 
               					 <c:if test="${row.value==userUnit.id.userstation}">selected="selected"</c:if>>
				...
				<tr>
					...
					<c:if test="${not empty userUnit.id.userrank}">						
						...
						<select  id="userrank"  name="userUnit.id.userrank" disabled="disabled" style="width:200px;height:30px;">
							<option value="">---请选择---</option>
					...
					<c:if test="${empty userUnit.id.userrank}">						
						...
						<select id="userrank"  name="userUnit.id.userrank" style="width:200px;height:30px;">
						<option value="">---请选择---</option>
							<c:forEach var="row" items="${cp:LVB('RankType')}">
               					 <option value="<c:out value='${row.value}'/>" 
               					 <c:if test="${row.value==userUnit.id.userrank}">selected="selected"</c:if>>
