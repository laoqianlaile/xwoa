...
<body>
		...
		<s:form action="optApply" namespace="/powerruntime"
			...
			<table cellpadding="0" cellspacing="0" align="center">
				...
				<%-- <tr>
					...
					<td><s:textfield name="s_proposer_Name"
							value="%{#parameters['s_proposer_Name']}" /></td>
					<td class="addTd">申请者机构代码:</td>
					<td><s:textfield name="s_proposer_Unitcode"
							value="%{#parameters['s_proposer_Unitcode']}"
							style="width:33%;height:25px;" /></td>
				</tr> --%>
				<tr >
					...
					<td><s:textfield name="s_powername"
							value="%{#parameters['s_powername']}" />
				...
				<tr class="searchButton">
					<td colspan="5"><s:submit method="listSQ" key="opt.btn.query"
							cssClass="btn" /></td>
				</tr>
			</table>
		</s:form>
	...
	<ec:table action="powerruntime/optApply!listSQ.do"
		...
		<ec:row>
			...
			<%-- <ec:column property="proposerType" title="申请者类别"
				style="text-align:center" sortable="false">
				${cp:MAPVALUE("PROPOSER_TYPE",srPermitApply.proposerType)}
				</ec:column>
			<ec:column property="proposerUnitcode" title="申请者机构代码"
				style="text-align:center" sortable="false">
			</ec:column>
			...
			<ec:column property="opt" title="操作" sortable="false"
				...
				<c:if test="${empty djid }">
					...
					<a
						href="${pageContext.request.contextPath}/powerruntime/optApply!generalOptView.do?djId=${srPermitApply.djId}&nodeInstId=0&s_itemtype=${s_itemtype}&applyItemType=${srPermitApply.applyItemType}">查看办件</a>
				</c:if>
				<c:if test="${not empty djid }">
					<a href='#' onclick="confirmSQ('${srPermitApply.djId}','${djid}')">关联事项</a>
				</c:if>
			</ec:column>
		</ec:row>
	</ec:table>
