...
<s:form action="generalOperator!saveStuffOfBookPage.do" method="POST"
					...
					<c:if test="${optStuffs != null}">
						<c:forEach var="info" items="${optStuffs}">
							...
							<c:if test="${info.iszhi ne '1'}">
								<a style="text-decoration: none; color: blue;" title="${info.filename}"  onclick="downFile('${info.stuffid}');return false;" >
									<c:choose>
						...
						<c:otherwise><c:out value="${info.filename}" /></c:otherwise>
							     ...
							     <c:if test="${empty isDelete || isDelete eq 'T' || uc eq info.uploadusercode   }">
									<a onclick='return confirm("是否确定删除？");'href="<%=request.getContextPath()%>/powerruntime/generalOperator!deleteStuffInfoOfBookPage.do?stuffInfo.stuffid=${info.stuffid}" style="color: red;">删除</a>
									</c:if>
								</c:if>
							</c:if>
						</c:forEach>
