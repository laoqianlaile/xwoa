 ...
 */
(function (J) {
	var top = window, doc, cover, ZIndex, install = false, ie6 = (J.browser.msie && J.browser.version < 7) ? true : false, getSrc = function () {
		if (J.browser.msie) {
			return (ie6 ? "" : "javascript:''");
		} else {
			return "javascript:void(0);";
		}
	}, iframeTpl = ie6 ? "<iframe hideFocus=\"true\" " + "frameborder=\"0\" src=\"" + getSrc() + "\" style=\"position:absolute;" + "z-index:-1;width:100%;height:100%;top:0px;left:0px;filter:" + "progid:DXImageTransform.Microsoft.Alpha(opacity=0)\"></iframe>" : "", compat = function (d) {
		d = d || document;
		return d.compatMode === "CSS1Compat" ? d.documentElement : d.body;
	}, getScrSize = function () {
		if ("pageXOffset" in top) {
			return {x:top.pageXOffset || 0, y:top.pageYOffset || 0};
		} else {
			var d = compat(doc);
			return {x:d.scrollLeft || 0, y:d.scrollTop || 0};
		}
	}, getDocSize = function () {
		var d = compat(doc);
		return {w:d.clientWidth || 0, h:d.clientHeight || 0};
	}, getZIndex = function () {
		if (!ZIndex) {
			ZIndex = 999;
		}
		...
		var rel = compat(doc);
		J(cover).css({width:Math.max(rel.scrollWidth, rel.clientWidth || 0) - 1 + "px", height:Math.max(rel.scrollHeight, rel.clientHeight || 0) - 1 + "px"});
	...
	J.dialog = function (opts, elem) {
		var S = this, r = S.opt = J.extend({title:"lhgdialog \u5f39\u51fa\u7a97\u53e3", width:400, height:300, id:"lhgdlgId", SetTopWindow:false, btns:true, link:false, page:"", event:"click", fixed:false, top:"center", left:"center", drag:true, resize:true, loadingText:"\u7a97\u53e3\u5185\u5bb9\u52a0\u8f7d\u4e2d\uff0c\u8bf7\u7a0d\u7b49..."}, opts || {});
		if (r.SetTopWindow) {
			top = r.SetTopWindow;
			doc = top.document;
		}
		...
		S.SetIFramePage = function () {
			...
			if (r.html) {
				...
				if (r.page) {
					innerDoc = ["<iframe frameborder=\"0\" src=\"", r.page, "\" scrolling=\"auto\" ", "id=\"lhgfrm\" style=\"display:none;width:100%;height:100%;\"></iframe>"].join("");
				}
			}
			dialogTpl = ["<div id=\"", r.id, "\" class=\"lhgdig\" style=\"width:", r.width, "px;height:", r.height, "px;\">", "<table border=\"0\" cellspacing=\"0\" cellpadding=\"0\">", "<tr>", "<td class=\"lhgdig_leftTop\"></td>", "<td id=\"lhgdig_drag\" class=\"lhgdig_top\">", "<div class=\"lhgdig_title\"><span id=\"lhgdig_icon\" class=\"lhgdig_icon\"></span>", r.title, "</div>", "<div id=\"lhgdig_xbtn\" class=\"lhgdig_xbtn\"></div>", "</td>", "<td class=\"lhgdig_rightTop\"></td>", "</tr>", "<tr>", "<td class=\"lhgdig_left\" id=\"lhgdigLeft\"></td>", "<td>", "<table border=\"0\" cellspacing=\"0\" cellpadding=\"0\">", "<tr>", "<td id=\"lhgdig_content\" class=\"lhgdig_content\">", innerDoc, "<div id=\"throbber\" class=\"lhgdig_throbber\"><span id=\"lhgdig_load\">", r.loadingText, "</span></div>", "</td>", "</tr>", r.btns ? "<tr><td id=\"lhgdig_btns\" class=\"\"></td></tr>" : "", "</table>", "</td>", "<td class=\"lhgdig_right\"></td>", "</tr>", "<tr>", "<td class=\"lhgdig_leftBottom\"></td>", "<td class=\"lhgdig_bottom\"></td>", "<td id=\"lhgdig_drop\" class=\"lhgdig_rightBottom\"></td>", "</tr>", "</table>", iframeTpl, "</div>"].join("");
			return dialogTpl;
		};
		S.ShowDialog = function () {
			...
			var fixpos = r.fixed && !ie6 ? "fixed" : "absolute", html = S.SetIFramePage();
			S.dg = J(html, doc).css({position:fixpos, zIndex:getZIndex()}).appendTo(doc.body)[0];
		...
		S.ShowCover = function () {
			...
			if (!cover) {
				var html = ["<div id=\"lhgdgCover\" style=\"position:absolute;top:0px;left:0px;", "background-color:#404040;\">", iframeTpl, "</div>"].join("");
		...
		S.iPos = function (dg, tp, lt, fix) {
			var cS = getDocSize(top), sS = getScrSize(top), dW = dg.offsetWidth, dH = dg.offsetHeight, x, y;
			if (fix) {
				if (ie6) {
		...
		S.SetDialog = function (dg) {
			...
			if (r.page.length > 0) {
				...
				J(S.dgFrm).bind("load", function () {
					if (!S.opt.link) {
						var indw = J.browser.msie ? S.dgWin.document : S.dgWin;
		...
		S.reContentSize = function (dg) {
			var tH = J("#lhgdig_drag", dg)[0].offsetHeight, bH = J("#lhgdig_drop", dg)[0].offsetHeight, xW = S.lhgDigxW, nH = r.btns ? J("#lhgdig_btns", dg)[0].offsetHeight : 0, iW = parseInt(dg.style.width, 10) - xW, iH = parseInt(dg.style.height, 10) - tH - bH - nH;
			J("#lhgdig_content", dg).css({width:iW + "px", height:iH + "px"});
			if (r.html) {
				J("#lhgdig_inbox", dg).css({width:iW + "px", height:iH + "px"});
			}
			S.SetLoadLeft();
		};
		S.SetLoadLeft = function () {
			var loadL = (J("#lhgdig_content", S.dg)[0].offsetWidth - J("#lhgdig_load", S.dg)[0].offsetWidth) / 2;
			J("#lhgdig_load", S.dg)[0].style.left = loadL + "px";
		};
		S.reDialogSize = function (width, height) {
			J(S.dg).css({"width":width + "px", "height":height + "px"});
			S.reContentSize(S.dg);
		};
		...
		S.initDrag = function (elem) {
			...
			J(elem).bind("mousedown", function (ev) {
				if (ev.target.id === "lhgdig_xbtn") {
					return;
				}
				cS = getDocSize(top);
				sS = getScrSize(top);
				var lt = S.dg.offsetLeft, tp = S.dg.offsetTop, dW = S.dg.clientWidth, dH = S.dg.clientHeight;
				curpos = S.opt.fixed ? {x:lt + sS.x, y:tp + sS.y} : {x:lt, y:tp};
		...
		S.initSize = function (elem) {
			...
			function moveHandler(ev) {
				var curcoor = {x:ev.screenX, y:ev.screenY};
				dialog = {w:curcoor.x - lacoor.x, h:curcoor.y - lacoor.y};
				if (dialog.w < 200) {
					dialog.w = 200;
				}
				if (dialog.h < 100) {
			...
			J(elem).bind("mousedown", function (ev) {
				...
				cS = getDocSize(top);
				sS = getScrSize(top);
				var lt = S.dg.offsetLeft, tp = S.dg.offsetTop;
				curpos = S.opt.fixed ? {x:lt + sS.x, y:tp + sS.y} : {x:lt, y:tp};
		...
		S.addBtn = function (id, txt, fn) {
			if (J("#" + id, S.dg)[0]) {
				...
				var html = "<a id=\"" + id + "\" class=\"lhgdig_button\" href=\"" + (ie6 ? "###" : "javascript:void(0)") + "\" hidefocus=\"true\"><em>" + txt + "</em></a>", btn = J(html, doc).bind("click", fn)[0];
	...
	J(function () {
		var lhgDY = setTimeout(function () {
			new J.dialog({id:"reLoadId", html:"lhgdialog", width:100, title:"reLoad", height:100, left:-9000}).ShowDialog();
			clearTimeout(lhgDY);
		}, 150);
	});
})(lhgcore);
