...
public class DashboardAction extends BaseAction {
    ...
    private List<VOaInformation> activityList;
    ...
    List<VoptIdeaInfo> optProcinfoList;
    private OptBaseInfoManager optBaseInfoManager;
    Map<String,String> optBaseExceptionNumMap;//异常办件数量
    Map<String,String> optBaseNumbyYearMap;//年度办件数量
 ...
 public String showAdminPage(){
        ...
        try {
            userRank=getUserRankByUsercode(((FUserDetail) getLoginUser()).getUsercode());
      //孤儿节点
            nodeList = flowManager.listNodesWithoutOpt();
        } catch (Exception e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
        }
        
        return "dashboardfoAdmin";

    }
 
//年度办件统计
 public String showOptBaseNumByYear(){
     List<Object[]> l=optBaseInfoManager.listoptBaseNumByYear();
     optBaseNumbyYearMap=new HashMap<String, String>();
     if(l!=null&&l.size()>0){
         for(Object [] o:l){
             optBaseNumbyYearMap.put(o[1].toString(), o[2].toString());
         }
     }
     return "optBaseNum";
 }
 //办件异常统计
 public String showOptBaseExceptionNum(){
     List<Object[]> l=optBaseInfoManager.listoptBaseExceptionNum();
     optBaseExceptionNumMap=new HashMap<String, String>();
     if(l!=null&&l.size()>0){
         for(Object [] o:l){
             optBaseExceptionNumMap.put(o[1].toString(), o[2].toString());
         }
     }
     return "exceptionnum";
 }
 //今日办件痕迹
 public String showToDayOptProcinfo(){
     optProcinfoList= optProcInfoManager.listIdeaLogsBytoday();
     if(optProcinfoList!=null&&optProcinfoList.size()>0){
         for(int i=0;i<optProcinfoList.size()&&i<=5;i++){
             String type=StringUtils.substringBefore(optProcinfoList.get(i).getDjId(), "0");
             optProcinfoList.get(i).setItemTypeName(CodeRepositoryUtil.getValue("oa_ITEM_TYPE", type));
         } 
     }
     
     return "todayprocinfo";
 }
    ...
    public void setDashboardInfo(String usercode){
       ...
       // userRank = this.getUserrank(usercode);
        ...
        request.setAttribute("todoTaskNum", dashboardManager.getToDoTaskNum(usercode));//待我办理
        ...
        //request.setAttribute("unReadEmailNum", dashboardManager.getUnReadEmailNum(usercode));
        //request.setAttribute("lwTaskNum", dashboardManager.getLWTaskNum(usercode));
        request.setAttribute("qbTaskNum", dashboardManager.getQBTaskNum(usercode));
       // request.setAttribute("dcdbTaskNum", dashboardManager.getDCDBTaskNum(usercode));
        
       //判断角色是否为主席，如果是发文、收文  变为  “审阅”、“审批"
        ...
        //showTX();//消息提醒
        //showJWBL();//经我办理
        //Tzgg_dashboard();//通知公告列表
        ZYTZ_dashboard();//重要通知
        //activity_dashboard();//活动
        //待办列表数目显示
       Map<String,String> numberMap = dashboardManager.getvusertasklistNum(usercode);
...
//通过ajax获取首页未读数
    public String ajaxDashboardNum(){
        ...
        if (uinfo != null && !StringUtils.isBlank(uinfo.getUsercode())) {
            ...
            ajaxDashboardNumMap.put("unReadEmailNumSpan", dashboardManager.getUnReadEmailNum(usercode));
    ...
    private void Tzgg_dashboard() {
        Map<String,Object> filterMap = new HashMap<String,Object>();
        PageDesc pageDesc =  new PageDesc(1, 5); 
        filterMap.put("state", "1");
        filterMap.put("besides_info_Type", "activity");
        tzggList=voaInformationManager.listObjects(filterMap, pageDesc);
    }
    //活动
    @SuppressWarnings("unchecked")
    private void activity_dashboard() {
        Map<String,Object> filterMap = new HashMap<String,Object>();
        PageDesc pageDesc =  new PageDesc(1, 5); 
        filterMap.put("state", "1");
        filterMap.put("info_Type", "activity");
        activityList=voaInformationManager.listObjects(filterMap, pageDesc);
    }
    //重要通知公告
    private void ZYTZ_dashboard(){
        Map<String,Object> filterMap = new HashMap<String,Object>();
        PageDesc pageDesc =  new PageDesc(1, 5); 
        filterMap.put("state", "1");
        filterMap.put("istop","T" );
        List<VOaInformation> zytzList=voaInformationManager.listObjects(filterMap, pageDesc);
        request.setAttribute("zytzList", zytzList);
    }
     ...
     * 展示领导日程安排
     */
    public void viewScheduleOfLeader(){
       String strYear = request.getParameter("year");
       String strWeekNo = request.getParameter("weekNo");
       JSONObject jo = dashboardManager.getScheduleOfLeaders(strYear==null?null:Integer.parseInt(strYear), strWeekNo==null?null:Integer.parseInt(strWeekNo));
       try{
           ServletActionContext.getResponse().getWriter().print(jo.toJSONString());    
       }catch(Exception e){
...
//                      VuserTaskListOA oa=objList.get(i);
   ...
   /*//通知公告-日历 
    ...
    //经我办理的
    private void showJWBL(){
        List<VOptBaseList> optBaseList = dashboardManager.findCaseHandledBySelf(5, getLoginUserCode());
        for(int i=0;i<optBaseList.size();i++){
            VOptBaseList oa=optBaseList.get(i);
        ...
        request.setAttribute("casesHandleBySelf", optBaseList);
    }
    /**
     * 查询添加到首页的业务入口
     * @return
...
public String sydb_new(){
      ...
      //filterTasks(vuserTaskListrl,userCode);
      if(vuserTaskListrl.size()>10){
          vuserTaskListrl =(List<VuserTaskListOA>) getSubList(vuserTaskListrl,11);
      }
      if(vuserTaskListrl!=null&&vuserTaskListrl.size()>0){
          //判断objList是否是督办流程的列表数据
                for(int i=0;i<vuserTaskListrl.size();i++){
...
/*      if(vuserTaskListrl.size()>10)
    ...
    //分管领导 审核顺序 过滤
    private List<VuserTaskListOA> filterTasks(List<VuserTaskListOA> vuserTaskListOAList,String userCode){
...
public String calendarSX(){
    ...
    public List<VoptIdeaInfo> getOptProcinfoList() {
        return optProcinfoList;
    }

    public void setOptProcinfoList(List<VoptIdeaInfo> optProcinfoList) {
        this.optProcinfoList = optProcinfoList;
    }

    public Map<String, String> getOptBaseNumbyYearMap() {
        return optBaseNumbyYearMap;
    }

    public void setOptBaseNumbyYearMap(Map<String, String> optBaseNumbyYearMap) {
        this.optBaseNumbyYearMap = optBaseNumbyYearMap;
    }
    ...
    public List<VOaInformation> getActivityList() {
        return activityList;
    }

    public void setActivityList(List<VOaInformation> activityList) {
