...
//            dispatchDocList = dispatchDocListManager.listObjects(" FROM VDispatchDocList where 1=1", filterMap, pageDesc);
    ...
    public String listV2() {
        try {
            ...
            dispatchDocList = dispatchDocListManager.listDispatchDoc(filterMap, pageDesc);
...
//                  filterMap.put("unitcode",userUnit.getUnitcode());//部门编码从页面传入s_unitcode
    ...
    public void exportExcelByPo() throws IOException {
     ...
     // 获取需要生成Excel的数据
        ...
        String[] header = {"序号","发文号","文件标题","发文时间","经办人","签发人","主送单位","办理步骤","办理人员"};// 列头
        //String[] header = {"序号","发文号","文件标题","主送单位","经办人","签发人","发文时间"};// 列头
       BizCommUtil.doPoi2Excel("发文信息", header, chosedList);
    }
        ...
        if (null != object.getItemId()) {
            ...
            if (null != vPowerOrgInfo) {
                if ("T".equals(vPowerOrgInfo.getIsgeneralmodule())) {
                    ...
                    this.initModuleFWData(vPowerOrgInfo.getGeneralmodulecode(),vPowerOrgInfo.getAgencyName());
                }
    ...
    private void createPDF(DispatchDoc dispatchDoc,String nodeName,Long nodeInstId,String userCode,String exeToolpath,String formHtmlUrl){
        ...
        try {
           ...
           File pdfStoreFile = new File(pdfStorePath);
            //创建pdf
           optPdfInfo = optPdfInfoManager.createPDF2(dispatchDoc.getDjId(),nodeInstId,formHtmlUrl);
            //转成swf
    ...
    public String saveAndSubmitDispatchDoc() {
        ...
        if (optBaseInfo.getFlowInstId() == null
            ...
            if (flowCode == null || flowCode.trim().length() == 0) {
                // 获取事权对应的信息
                VPowerOrgInfo vPowerOrgInfo = vPowerOrgInfoManager.getSupPowerInfo(optBaseInfo.getPowerid(), fuser.getPrimaryUnit());
                flowCode = vPowerOrgInfo.getWfcode();
                d_itemtype="FW";//20181105添加跳转条件使用
            }
...
//            super.saveTeamRolepeople(fuser.getUsercode(), "csnw");
            ...
            super.saveTeamRole( "fwjbr",fuser.getUsercode());
        }
      ...
      //开启一个线程
        if("true".equals(SysParametersUtils.getParameters("generatePdfForDocEnable", "false"))){
            ...
            taskExecutor.execute(new Runnable(){
                @Override
                public void run() {
                    createPDF(object, "发文发起", 0L,userCode,exePath,formHtmlUrl);
                }
            });
        }
...
//        request.setAttribute("incomeDocList", getRelativeIncomes(djId)); /* 关联收文 */
    ...
    private void getPermitInfo() {
        ...
        object.setDraftUserName(StringUtils.isBlank(object.getDraftUserName())?fuser.getUsername():object.getDraftUserName());
        object.setOptUnitName(StringUtils.isBlank(object.getDraftUserName())?CodeRepositoryUtil.getValue("unitcode", fuser.getUnitName()):object.getOptUnitName());  
    }
    ...
    public String viewDispatchDocInfo() {
        try {
            ...
            if (dispatchDoc != null) {
                ...
                object.setOptBaseInfo(optBaseInfoManager.getObjectById(object.getDjId()));
            }else{
    ...
    public String viewDispatchDocInfoTemplate() {
        viewDispatchDocInfo();
        object.setDispatchDate(DatetimeOpt.convertDateToString(object.getDispatchdate()));
        return "viewTemplate";
         ...
         //                  否则  只保留上下文 /oa_szgh
              ...
              if(null!=ideas&&ideas.size()>0){
                  for(int i=0;i<ideas.size();i++){
                          request.setAttribute("IDEA_"+i,ideas.get(i).getTranscontent().replace(relaceTag, contextPath) );
                  }
              }
        }
    ...
    * private void writeIdeaModuleToRequestAttribute(String djId,boolean needValid){
        ...
        String[]MODULE_CODE_ARR_pt = new String[]{"fhptfw_bmsh","fhptfw_bmhq","fhptfw_zyld","fhptfw_dy"};
        String[]MODULE_CODE_ARR_sf = new String[]{"fhsffw_bmsh","fhsffw_bmhq","fhsffw_zyld","fhsffw_dy"};
        String[]MODULE_CODE_ARR_kb = new String[]{"fhkbfw_bmsh","fhkbfw_bmhq","fhkbfw_zyld","fhkbfw_dy"};
        final String[]MODULE_CODE_ARR =(String[]) ArrayUtils.addAll((String[]) ArrayUtils.addAll(MODULE_CODE_ARR_pt, MODULE_CODE_ARR_sf), MODULE_CODE_ARR_kb);
    ...
    private void saveCommon() {
        ...
        if (baseInfo == null || baseInfo.getCreatedate() == null) {
            ...
            //Headunitcode接收部门业务处室--收发文层级发文vPowerOrgInfo.getAgencyName()时存放对应的处理部门unitcode
              ...
              // 获取事权对应的信息
              VPowerOrgInfo vPowerOrgInfo = vPowerOrgInfoManager.getSupPowerInfo(object.getItemId(), user.getPrimaryUnit());
              //获取上级机构配置信息
