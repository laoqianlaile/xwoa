...
public class IODocCommonBizAction <T> extends PowerRuntimeEntityAction<T> implements ServletResponseAware {
     ...
     */
      ...
      */
       ...
       */
        ...
        * @return
        */
       public OptHtmlFrameInfo  getPreviousNode(String djId) {
           OptHtmlFrameInfo previousNodeFrameInfo = new OptHtmlFrameInfo();
           previousNodeFrameInfo.setFrameLegend("前一节点办理信息");
           previousNodeFrameInfo.setFrameId("previousNodeFrame");
           previousNodeFrameInfo
                   .setFrameSrc("/powerruntime/generalOperator!listPreNode.do?djId=" +djId+"&nodeInstId="+curNodeInstId);
          return previousNodeFrameInfo;
      }
     ...
     * 目前支持 人员选择（直接指定到人）  部门选择（接收部门） 支持配置文书类型
     * 发文机构名义：分级管理
     * 
     */
    protected  void  initModuleFWData(String  modulecode,String  agencyname){
        
        initModuleData(  modulecode);
        /**
         * 根据是否可以生产公文 ，确定需要编辑的文档模板
         */
        this.initTemplateConfig(agencyname);  
        
        /**
         * 筛选文号
         */
        
        this.initZWH(agencyname);
    }
     ...
     * 
     */
    protected  void  initModuleData(String  modulecode){
        ...
        if ("T".equals(moduleParam.getHasOrgRole())) {
            ...
            super.initPFUnit(moduleParam.getXbOrgRoleCode(), null);
     ...
     * 根据是否可以生产公文 ，确定需要编辑的文档模板
     * 优先级：在配置模块勾选 编辑文书时
     * 1.文书类别 null   文书模板  null   --》查询分级配置中该部门文书
     * 2.文书类别 有值   文书模板  null   --》查询该类别文书
     * 3.文书类别 null   文书模板  有值   --》查询选中的文书模板
     * （1实现以部门名义发文时待选部门文书  ；2,3保持原来逻辑实现以顶级部门发文时公共的文书可以不用重复配置；）
     * 比起单纯的约定分级配置的优先级这样做有更多的可能性
     *  
     */
    protected void initTemplateConfig(String agencyname) {
        if (moduleParam != null && moduleParam.getHasDocument() != null
            ...
            if(null==templateList||templateList.size()==0){
                ...
                if (!StringUtils.isBlank(templates)) {
                ...
                } else if(!StringUtils.isBlank(moduleParam.getDocumentType())){
                    ...
                    if(!"0".equals(unitParent.getParentunit())){//顶级部门
                         unitcode="01".equals(agencyname)?unitParent.getParentunit():unitcode;
                    }
                   
                    templateList=oaFwdepmangeManager.getDocListByUnit(unitcode);
                }
            }
     ...
     *准备有效文号规则 
     */
    protected void initZWH(String agencyname) {
