...
public class IncomeDocAction extends IODocCommonBizAction<IncomeDoc> implements ServletResponseAware {
    ...
    private VPowerOrgInfoManager vPowerOrgInfoManager;//事项部门管理视图
    /*-------Temp 收文  --------*/
        ...
        try {
           ...
           if(fuser==null && StringUtils.isBlank(usercode)){
               ...
               if(StringUtils.isNotBlank(usercode)){
                    fuser=sysUserManager.loadUserByUsername(usercode);
               }
           }
                ...
                if (StringUtils.isBlank(object.getDjId())) {
                    ...
                    object.setDjId(incomeDocMgr.getNextkey(power.getItemType()));    //收文流水号
                ...
                // 如果没有编号则自动查询
                if (StringUtils.isBlank(object.getAcceptArchiveNumber())) {
                    if("XW-SW-0001".equals(object.getItemId()))
                    object.setAcceptArchiveNumber(incomeDocMgr.getNewCode());// 获取自动生成的编号 管委会
                    if("XW-SW-0002".equals(object.getItemId()))
                        object.setAcceptArchiveNumber(incomeDocMgr.getNewCodeByAJJ());//安监局
                    if("XW-SW-0003".equals(object.getItemId()))
                        object.setAcceptArchiveNumber(incomeDocMgr.getNewCodeByHBJ());//环保局
                    if("XW-SW-0004".equals(object.getItemId()))
                        object.setAcceptArchiveNumber(incomeDocMgr.getNewCodeByDQ());//党群
                    if("XW-SW-0005".equals(object.getItemId()))
                        object.setAcceptArchiveNumber(incomeDocMgr.getNewCodeByAJ());//安委会
                    if("XW-SW-0008".equals(object.getItemId()))
                        object.setAcceptArchiveNumber(incomeDocMgr.getNewCodeByGHJSQ());//规划建设局
                    if("XW-SW-0013".equals(object.getItemId()))
                        object.setAcceptArchiveNumber(incomeDocMgr.getNewCodeByTY());//通用
                    if("XW-SW-0014".equals(object.getItemId()))
                        object.setAcceptArchiveNumber(incomeDocMgr.getNewCodeByGZB());//国资办
                }
                //初始化基本信息
                ...
                if(optBaseInfo==null){
                    ...
                    if(StringUtils.isBlank(power.getItemId())){
                        super.postAlertMessage("操作失败，请检查权力部门配置！", response);
                        return null;
                    }
                    
                    flowCode=power.getOptFlowCode();
              ...
              //准备通用配置数据
                if(null!=object.getItemId()){
                    // 获取事权对应的信息
                    VPowerOrgInfo vPowerOrgInfo= vPowerOrgInfoManager.getSupPowerInfo(object.getItemId(), fuser.getPrimaryUnit()); 
                    if(null!=vPowerOrgInfo ){
                        if("T".equals(vPowerOrgInfo.getIsgeneralmodule())){
                            curdjId=object.getDjId();
                            moduleCode=vPowerOrgInfo.getGeneralmodulecode();
                            this.initModuleData(vPowerOrgInfo.getGeneralmodulecode());  
                        }
                        request.setAttribute("groupid", vPowerOrgInfo.getGroupid());
                    }
...
//            String swh=incomeDocType+incomeDeptType+"11";
    ...
    public String saveRegisterDoOrRead(){
        try {
          ...
          /*  if (StringUtils.isBlank(optBaseInfo.getAcceptArchiveNo())) {
            ...
            //Headunitcode接收部门业务处室--收发文层级发文vPowerOrgInfo.getAgencyName()时存放对应的处理部门unitcode
            
            // 获取事权对应的信息
            VPowerOrgInfo vPowerOrgInfo = vPowerOrgInfoManager.getSupPowerInfo(object.getItemId(), fuser.getPrimaryUnit());
            //获取上级机构配置信息
              FUnitinfo  unitParent =sysUnitManager.getObjectById(fuser.getPrimaryUnit());
              if(null!=fuser){
                  String unitcode=fuser.getPrimaryUnit();
                  if(!CommonOptCodeUtil.TOP_UNITCODE.equals(unitParent.getParentunit())){//顶级部门
                       unitcode=CommonOptCodeUtil.GW_NATURE_SUP.equals(vPowerOrgInfo.getAgencyName())?unitParent.getParentunit():unitcode;
                  }
                  optBaseInfo.setHeadunitcode(unitcode);
                  optBaseInfo.setGwNature(vPowerOrgInfo.getAgencyName());
              }
            ...
            optBaseInfo.setToBeanfinishedDate(object.getToBeanfinishedDate());//办结日期
        ...
        if("true".equals(SysParametersUtils.getParameters("generatePdfForDocEnable", "false"))){
          ...
          //"***/anonymous***"代表匿名访问
            ...
            taskExecutor.execute(new Runnable(){
                @Override
                public void run() {
                    createPDF(object, "收文登记", 0L,userCode,exePath,formHtmlUrl);
                }
            });
        }
        return this.list();
    ...
    public String acceptArchiveNoVali(){
        ...
        try {
            List<OptBaseInfo> opt=optBaseInfoManager.listOptBaseInfo(acceptArchiveNo, itemId);
...
private List<OptHtmlFrameInfo> getAllInfoListFrame(List<OptHtmlFrameInfo> frameList,String djId) {
    ...
    /*-------Temp 收文   end--------*/
     ...
     public void exportExcelByPo() throws IOException {
      ...
      // 获取需要生成Excel的数据
         ...
         String[] header = {"序号","收文号","文件标题","来文单位","拟办意见","收文时间"};// 列头
        BizCommUtil.doPoi2Excel("收文信息", header, chosedList);
     }
     ...
     public void exportExcelByPoV2() throws IOException {
      ...
      // 获取需要生成Excel的数据
         ...
         String[] header = {"序号","收文号","文件标题","发文单位","发文字号","收文时间"};// 列头
        BizCommUtil.doPoi2Excel("收文信息", header, chosedList);
     }
     ...
     */
    @SuppressWarnings({ "unchecked", "rawtypes" })
    public String registerDoOrReadEditNew() {
        try {
           ...
           if(fuser==null && StringUtils.isBlank(usercode)){
               ...
               if(StringUtils.isNotBlank(usercode)){
                ...
                // 如果没有编号则自动查询
                if (StringUtils.isBlank(object.getAcceptArchiveNumber())) {
                    object.setAcceptArchiveNumber(incomeDocMgr.getNewCode());// 获取自动生成的编号
                }
                //初始化基本信息
                OptBaseInfo optBaseInfo = optBaseInfoManager.getObjectById(object.getDjId());
        ...
        return "registerDoOrReadEditNew";
    }
    ...
    public String saveAndSubmitIncomeDoc() {
        ...
        if (null != fuser) {
            // 发送短信给下一步操作人
            oaSmssendManager.saveFlowMsgs(
                    request.getParameter("isSendMessage"), fuser.getUsercode(),
                    nextNode);
            oaSmssendManager.executeSendMsg();
        }
...
//            }
  ...
  /*  public String deleteShip(){
   ...
   /* public String saveShip(){
    ...
    public String registerDoOrReadView() {
       ...
       if(fuser==null && StringUtils.isBlank(usercode)){
        ...
        request.setAttribute("canEdit", request.getParameter("canEdit"));
        // 设置在办件的超时提醒
        Map<String, Object> filterMap = new HashMap<String, Object>();
        filterMap.put("djId", object.getDjId());
      ...
      //归档信息
        ...
        //特殊处理动态加载意见
        writeIdeaModuleToRequestAttributeFree(object.getDjId(),isVailViewPower);
        //动态加载意见
   ...
   //                  否则  只保留上下文 /oa_szgh
     ...
     * @param needValid
     */
    private void writeIdeaModuleToRequestAttributeFree(String djId,boolean needValid) {
        String isPdf = request.getParameter("isPdf");
        boolean showIdeaContent = false;
        // 如果需要验证，那么检测是否有查看意见的权限
        boolean hasViewPower = CodeRepositoryUtil.checkUserOptPower("DOCXZ","DocViewAll");
        ...
        List<OptIdeaInfo> ideas = ideaTempletManager.loadAllAvailableIdeasXW(djId, showIdeaContent);
        String relaceTag = SysParametersUtils.getParameters("mobileTerminalAccessAddr", null);
        String contextPath = "T".equals(isPdf) ? relaceTag : "";
        if (null != ideas && ideas.size() > 0) {
            for (OptIdeaInfo idea : ideas) {
                if(idea.getTranscontent() == null){
                    continue;
                }
                idea.setTranscontent(idea.getTranscontent().replace(relaceTag, contextPath));
            }
