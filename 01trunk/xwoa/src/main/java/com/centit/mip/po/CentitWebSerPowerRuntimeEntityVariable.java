...
public class CentitWebSerPowerRuntimeEntityVariable<T extends Object>
        implements SysVariableTranslate {
    protected T object;
    protected static final Log log = LogFactory
            .getLog(CentitWebSerPowerRuntimeEntityVariable.class);
    ...
    public CentitWebSerPowerRuntimeEntityVariable() {
        object = null;
        optObject = null;
        requestParams = null;
    }
        ...
        try {
            ...
            for (int i = 0; i < m.length; i++) {
                if (m[i].getName().indexOf("get") == 0) {
                    requestParams.put(m[i].getName(),
                            HtmlFormUtils.getParameterString(m[i].invoke(obj,
                                    new Object[0])));
                }
            }
        } catch (Throwable e) {
    ...
    public String getVarValue(String lableName) {
        ...
        if (optObject != null) {
            try {
                Object obj = ReflectionOpt.forceGetProperty(optObject,
                        lableName);
                if (obj != null) {
                    s = obj.toString();
                    if (!StringRegularOpt.isNumber(s))
