...
public class WSClientManagerImpl implements WsclientManager {
    ...
    public String syncUserList(String userInfos) {
        String clientUrl = CodeRepositoryUtil.getValue("SYSPARAM",
                "MOBLIEPHONESER");
        try {
            if (StringUtils.isNotBlank(userInfos)) {
                ...
                if (null != userList && userList.size() > 0) {
                    for (FUserinfo userinfo : userList) {
                        OaUserinfo oainfo = oaUserinfoManager
                                .getObjectById(userinfo.getUsercode());

                      //String unitcode = CodeRepositoryUtil.getPrimaryUnit(userinfo.getUsercode());//废弃次方法获取机构代码
                        String unitcode ="";
                        FUserunit fUserunit=sysunitdao.getUserPrimaryUnit(userinfo.getUsercode());
                        if(fUserunit!=null){
                            unitcode=fUserunit.getUnitcode();
                        }else{
                            unitcode=CodeRepositoryUtil.getPrimaryUnit(userinfo.getUsercode()); 
                        }
                        String unitname = CodeRepositoryUtil.getValue(
                                "unitcode", unitcode);
                        Client c = new Client(new URL(clientUrl));
                        Object[] result = c
                                .invoke("synchronizeUserInfo",
                                        new Object[] {
                                                userinfo.getUsercode(),   //用户内部代码
                                                userinfo.getLoginname(),  //用户登录名
                                                userinfo.getUsername(),   //用户中文名
                                                unitcode,                 //机构内部代码
                                                unitname,                 //机构名称
                                                userinfo.getUserpin(),    //用户凭证（加密密码）
                                                null == oainfo ? "" : oainfo
                                                        .getWorkplace(),  //工作地点
                                                null == oainfo ? "" : oainfo
                                                        .getTelephone(),  //办公电话
                                                ...
                                                null == oainfo ? "" : oainfo
                                                        .getSex(),         //性别
                                               null== oainfo?"":userinfo.getUserdesc(),  //增加用户描述（主要描述用户的职位等信息）
                                               null== oainfo?0L:userinfo.getSortid(),  //用户排序
                                                "T".equals(userinfo
                                                        .getIsvalid()) ? "0"
                                                        : "1" });// 0启用 1 禁用
                        log.info("用户信息:" + userinfo.getUsername()+userinfo.getUserpin());
                    }
                }
            }
        } catch (MalformedURLException e) {
            ...
            log.info("用户同步失败:请核对地址" + clientUrl);
            return "FALSE";
                ...
                if (null != unitList && unitList.size() > 0) {
                    for (FUnitinfo unitinfo : unitList) {
                        ...
                        Object[] result = c.invoke(
                                "synchronizeDeptInfo",
                                new Object[] { unitinfo.getUnitcode(),
                                        unitinfo.getParentunit(),
                                        unitinfo.getUnitname(),unitinfo.getUnitorder() });
                    }
                }
            }
        } catch (MalformedURLException e) {
