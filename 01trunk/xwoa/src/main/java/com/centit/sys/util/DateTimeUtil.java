 ...
 */
    public static String smartCalcSpanDays(java.util.Date beginDate, java.util.Date endDate){
        String defaultDateTag = "今天";//返回字符串  下个月 ，下周，明天，今天，昨天，本周，上周，本月，上月
        int spanDays=DatetimeOpt.calcSpanDays(DatetimeOpt.truncateToDay(beginDate),DatetimeOpt.truncateToDay(endDate));//算出俩个日期的时间差yyyy-mm-dd
        int nWeekDay= DatetimeOpt.getDayOfWeek(endDate);//一周中的第几天，所以他会受到 第一天是星期几 的影响。
        if(nWeekDay==0){//周日为0
            nWeekDay=7;
        }
...
//            spanDays  过去的时间
            if(spanDays>0&&spanDays<4){//昨天，前天
               ...
               if(spanDays>0&&spanDays<=nWeekDay){//本周
...
//          spanDays 未来的时间
            if(spanDays>0&&spanDays<4){//明天，后天
                ...
                if(spanDays>0&&spanDays<=(8-nWeekDay)){//本周
                defaultDateTag= getDayOfWeekShort(beginDate);
 ...
 */
     ...
     * @return
     */
    public static String getDecOfWeek(java.util.Date beginDate, java.util.Date endDate) {
        String [] weeklist = {"周日","周一","周二","周三","周四","周五","周六","",};
        return weeklist[DatetimeOpt.getDayOfWeek(beginDate)];
    }
    
    public static String smartDays(java.util.Date beginDate, java.util.Date endDate){
        String defaultDateTag = "今天";//返回字符串  下个月 ，下周，明天，今天，昨天，本周，上周，本月，上月
...
//          spanDays 未来的时间
            if(spanDays>0&&spanDays<4){//明天，后天
                ...
                if(spanDays>0&&spanDays<=(7-nWeekDay)){//本周
                defaultDateTag= getDayOfWeekShort(beginDate);
