...
public class RoleInfoDao extends BaseDaoImpl<FRoleinfo> {
    ...
    {
        return getHibernateTemplate().find("FROM FRolepower rp where rp.id.rolecode=?",rolecode);
    ...
    {
        this.doExecuteHql("DELETE FROM FRolepower rp where rp.id.rolecode=?", rolecode);
        
    }   
    
    public void saveRolePowers(List<FRolepower> rolePowers)
    {
        if( rolePowers.size() < 1) return;
    ...
    public List<FOptWithPower> getOptWithPowerUnderUnit(String sUnitCode)
    {
        String sSqlsen ="select a.optid,a.PreOptID,a.OptName,b.optcode "+
                ...
                "where b.optcode in " +
                    "(select optcode from f_rolepower where rolecode like '%$"+ sUnitCode+"') " +
                "order by a.optid";
    ...
    public List<Object> getRoleOptdef(String rolecode) {
        ...
        return this.getHibernateTemplate()
                .findByNamedParam(hql, new String[] { "rolecode" }, new Object[] { rolecode });
    }
}
