...
//            jspInfo.setTitle("督办查看");
   ...
   private List<WfTeam> WfTeamlist;
        ...
        **/
         ...
         * 简单的督办发起
         * 1.在办理过程中支持发起督办,支持编辑
         * 2.发起督办后在optbaseinfo中存储督办信息（是否督办）不触发督办流程
         * 3.督办件只是一个状态显示，不影响业务提交
         */
        public String flowEdit() {
            String supdjid=request.getParameter("supDjid");
            if(StringUtils.isNotBlank(supdjid)){
                OptBaseInfo optBaseInfo = optBaseInfoManager.getObjectById(supdjid);
                object.setLimittime(optBaseInfo.getFlowSuperviseDate());
                object.setAdvice(optBaseInfo.getFlowSuperviseIdea());
            }
            return "flowEdit";
        }
         ...
         * @return
         */
        public String flowSaveAndSubmit() {
            // 保存
            FUserDetail fuser = ((FUserDetail) getLoginUser());

             OptBaseInfo optBaseInfo = optBaseInfoManager.getObjectById(object.getSupDjid());
             optBaseInfo.setFlowSupervise("T");
             optBaseInfo.setFlowSuperviseDate(object.getLimittime());
             optBaseInfo.setFlowSuperviseIdea(object.getAdvice());
             optBaseInfoManager.saveObject(optBaseInfo);
