...
public class OaEquipmentpurchaseAction  extends OACommonBizAction<OaEquipmentpurchase>  {
	private static final Log log = LogFactory.getLog(OaEquipmentpurchaseAction.class);
	private static final long serialVersionUID = 1L;
	private OaEquipmentpurchaseManager oaEquipmentpurchaseMag;
	public void setOaEquipmentpurchaseManager(OaEquipmentpurchaseManager basemgr)
	{
		oaEquipmentpurchaseMag = basemgr;
		this.setBaseEntityManager(oaEquipmentpurchaseMag);
	}
	   ...
	   private VsuppowerinusingManager vsuppowerinusingManager;
	    ...
	    public String list() {
	        ...
	        objList = oaEquipmentpurchaseMag.listObjects(filterMap, pageDesc);
	    ...
	    public String edit() {
	        ...
	        if (StringUtils.isBlank(object.getDjId())) {
	            object.setDjId(optApplyManager.getNextDjId("SQ","oaEquipmentpurchase"));
	        }
	        if(StringUtils.isBlank(object.getCreater())){
	         // 新添加申请时，默认将系统登录人作为申请人
	            ...
	            if(null==object.getApplydate()){
                    object.setApplydate(new Date());
                }
                if(StringUtils.isBlank(object.getApplyuser())){
	   ...
	   //                  否则  只保留上下文 /oa_szgh
	    ...
	    public String savePermitReg() {
	        try {
	            ...
	            // 申请state=6标记为暂存
	            oaEquipmentpurchaseMag.saveObject(object);

	            object = oaEquipmentpurchaseMag.getObjectById(object.getDjId());
	    ...
	    public String saveAndSubmit() {
	        ...
	        // 保存
	        FUserDetail fuser = ((FUserDetail) getLoginUser());
	        OaEquipmentpurchase oaEquipmentpurchase = oaEquipmentpurchaseMag
	                .getObjectById(object.getDjId());
	        if (oaEquipmentpurchase != null) {
	            oaEquipmentpurchaseMag.copyObject(oaEquipmentpurchase, object);
	            object = oaEquipmentpurchase;
	        }
	        object.setCreater(fuser.getUsercode());
	         ...
	         if("true".equals(SysParametersUtils.getParameters("generatePdfForDocEnable", "false"))){
	           ...
	           //"***/anonymous***"代表匿名访问
	             ...
	             taskExecutor.execute(new Runnable(){
	                 @Override
	                 public void run() {
	                     optPdfInfoManager.createPDF(object.getDjId(), "设备购置（维修）", 0L,userCode,exePath,formHtmlUrl);
	                 }
	             });
	         }
	     }
	    ...
	    public String delete() {
	        if (StringUtils.isNotBlank(object.getDjId())) {
	            ...
	            oaEquipmentpurchaseMag.deleteObjectById(object.getDjId());
	        }
	        return "nextList";
	    }
	    ...
	    public String saveReditSQ() {
	        OaEquipmentpurchase oaEquipmentpurchase = oaEquipmentpurchaseMag
	                .getObjectById(object.getDjId());
	        if (oaEquipmentpurchase != null) {
	            oaEquipmentpurchaseMag.copyObjectNotNullProperty(
	                    oaEquipmentpurchase, object);
	            object = oaEquipmentpurchase;
	        }
	        oaEquipmentpurchaseMag.saveObject(object);
