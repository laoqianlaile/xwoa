...
public class OaArrangeweekManagerImpl extends
    ...
    @Override
    public List<OaArrangeweek> getDjidlist(String username) {//模糊搜索参加人员和参会领导
        // TODO Auto-generated method stub
        return  this.oaArrangeweekDao.getarrangeweeklist(username);
    }
    
    
    @Override
    public List<OaWorkSummary> getWorkSummaries(HashMap<String, Object> searchColumn) throws ParseException {
        ...
        String username = String.valueOf(searchColumn.get("username"));
        String np = String.valueOf(searchColumn.get("susername"));
        ...
        for (int i = 0; i < maxindex; i++) {
            ...
            if (i != 0) {
                createtime = DatetimeOpt.addDays(new SimpleDateFormat("yyyy-MM-dd").parse(firstDay), i);
            } else {
                createtime = new SimpleDateFormat("yyyy-MM-dd").parse(firstDay);
            }
            OaWorkSummary workSummary = new OaWorkSummary(createtime);//用于页面展示使用
            searchColumn.put("beginCreatetime", workSummary.getId()+ " 00:00:00");
            searchColumn.put("endCreatetime", workSummary.getId() + " 23:59:59");
            List<OaArrangeweek> arrangeweeks;
            String hql = "  From OaArrangeweek where 1=1  and ( attendusers like '%"+np+"%'  or attendleaders like '%"+np+"%'  )";
            if("owner".endsWith(showType)){
                searchColumn.remove("depno");
                 hql = "  From OaArrangeweek where 1=1  and ( attendusers like '%"+username+"%'  or attendleaders like '%"+username+"%'  )";
                arrangeweeks = oaArrangeweekDao.listObjects(hql,searchColumn);//本人
            }else{
                if(StringUtils.isNotEmpty(np) && !"null".equals(np)){
                    arrangeweeks = oaArrangeweekDao.listObjects(hql,searchColumn);
                }else{
                    arrangeweeks = oaArrangeweekDao.listObjects(searchColumn);//每天会议安排总数量
                }
            }
           
            for (int j = 0; j < arrangeweeks.size();) {
                OaArrangeweek oaArrangeweek = (OaArrangeweek) arrangeweeks.get(j);
