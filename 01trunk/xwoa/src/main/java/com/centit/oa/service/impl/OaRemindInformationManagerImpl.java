...
public class OaRemindInformationManagerImpl extends
        BaseEntityManagerImpl<OaRemindInformation> implements
        OaRemindInformationManager {
    private static final long serialVersionUID = 1L;
    public static final Log log = LogFactory
            .getLog(OaRemindInformationManager.class);
    ...
    public String getNextkey() {
        ...
        return "TX"
                + oaRemindInformationDao.getNextKeyBySequence(
                        "s_OaRemindInformation", 14);
    }
    ...
    public List<OaRemindInformation> recipientList(
        ...
        if (StringUtils.hasText(usercode)) {
            sHql += "and t.no in ("
                    + "select r.cid.addrbookid from AddressBookRelection r where r.cid.shareman= "
                    + HQLUtils.buildHqlStringForSQL(usercode);
            if (StringUtils.hasText(type)) {
                sHql += " and r.cid.bizType="
                        + HQLUtils.buildHqlStringForSQL(type);
            }
            sHql += ")   ";
        ...
        if (F_userBizopt_log != null && !"".equals(F_userBizopt_log)) {
            sHql += "and t.no not in ("
                    + "select f.dj_id from F_userBizopt_log f where f.createUser= "
                    + HQLUtils.buildHqlStringForSQL(F_userBizopt_log) + " ) ";
        }
        // =======================================================
        if (null != filterMap.get("orderbegtime")) {
            sHql += " order by begtime desc ";
        }
        List<OaRemindInformation> oaRemindInformationList = baseDao
                .listObjects(sHql, filterMap, pageDesc);
    ...
    public boolean sendOaRemindInformation(String djId, String sender,
        ...
        try {
            if (StringUtils.hasText(reser)) {
                ...
                bean.setEndtime(DatetimeOpt.addDays(
