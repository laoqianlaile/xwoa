...
public class OaKqreceptionAction  extends  OACommonBizAction<OaKqreception>  {
	private static final Log log = LogFactory.getLog(OaKqreceptionAction.class);
	private static final long serialVersionUID = 1L;
	private OaKqreceptionManager oaKqreceptionMag;
	public void setOaKqreceptionManager(OaKqreceptionManager basemgr)
	{
		oaKqreceptionMag = basemgr;
		this.setBaseEntityManager(oaKqreceptionMag);
	}
    ...
    public String list() {
        ...
        objList = oaKqreceptionMag.listObjects(filterMap, pageDesc);
    ...
    public String edit() {
        ...
        if (StringUtils.isBlank(object.getDjId())) {
            object.setDjId(optApplyManager.getNextDjId("SQ","oaKqreception"));
        ...
        if(StringUtils.isBlank(object.getCreater())){
         // 新添加申请时，默认将系统登录人作为申请人
            ...
            if(null==object.getApprotime()){
                object.setApprotime(new Date());
            }
   ...
   //                  否则  只保留上下文 /oa_szgh
    ...
    public String savePermitReg() {
        try {
            ...
            // 申请state=6标记为暂存
            oaKqreceptionMag.saveObject(object);

            object = oaKqreceptionMag.getObjectById(object.getDjId());
    ...
    public String saveAndSubmit() {
        ...
        // 保存
        FUserDetail fuser = ((FUserDetail) getLoginUser());
        OaKqreception oaKqreception = oaKqreceptionMag
                .getObjectById(object.getDjId());
        if (oaKqreception != null) {
            oaKqreceptionMag.copyObject(oaKqreception, object);
            object = oaKqreception;
        }
        object.setCreater(fuser.getUsercode());
        ...
        if (object != null) {
            ...
            if (StringUtils.isNotBlank(object.getRecievedepno())) {
                ...
                flowEngine.assignFlowOrganize(optBaseInfo.getFlowInstId(),
                        "JSBM", vPowerUserInfo.getUnitcode(), "接待清单接收部门");
            }
         ...
         if("true".equals(SysParametersUtils.getParameters("generatePdfForDocEnable", "false"))){
           ...
           //"***/anonymous***"代表匿名访问
             ...
             taskExecutor.execute(new Runnable(){
                 @Override
                 public void run() {
                     optPdfInfoManager.createPDF(object.getDjId(), "接待清单", 0L,userCode,exePath,formHtmlUrl);
                 }
             });
         }
     }
    ...
    public String delete() {
        if (StringUtils.isNotBlank(object.getDjId())) {
            ...
            oaKqreceptionMag.deleteObjectById(object.getDjId());
        }
        return "nextList";
    }
    ...
    public String saveReditSQ() {
        OaKqreception oaKqreception = oaKqreceptionMag
                .getObjectById(object.getDjId());
        if (oaKqreception != null) {
            oaKqreceptionMag.copyObjectNotNullProperty(
                    oaKqreception, object);
            object = oaKqreception;
        }
        oaKqreceptionMag.saveObject(object);
