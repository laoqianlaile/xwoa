 ...
 * @version
 */
public class OaKqnotificationAction  extends  OACommonBizAction<OaKqnotification>  {
	private static final Log log = LogFactory.getLog(OaKqnotificationAction.class);
	private static final long serialVersionUID = 1L;
	private OaKqnotificationManager oaKqnotificationMag;
	public void setOaKqnotificationManager(OaKqnotificationManager basemgr)
	{
		oaKqnotificationMag = basemgr;
		this.setBaseEntityManager(oaKqnotificationMag);
	}
	    ...
	    public String list() {
	        ...
	        objList = oaKqnotificationMag.listObjects(filterMap, pageDesc);
	    ...
	    public String edit() {
	        ...
	        if (StringUtils.isBlank(object.getDjId())) {
	            object.setDjId(optApplyManager.getNextDjId("SQ","oaKqnotification"));
	        }
	        ...
	        if(StringUtils.isBlank(object.getCreater())){
	         // 新添加申请时，默认将系统登录人作为申请人
                ...
                if(null==object.getKqtime()){
                    object.setKqtime(new Date());
                }
                if(StringUtils.isBlank(object.getKqdepname())){
                    object.setKqdepname(user.getPrimaryUnit());
                }
	        }
	        
	      //如果没有期号则自动查询
	         if(StringUtils.isBlank(object.getPeriods())){
	             object.setPeriods(oaKqnotificationMag.getNextKeyMax());
	         }
	   ...
	   //                  否则  只保留上下文 /oa_szgh
	    ...
	    public String savePermitReg() {
	        try {
	            ...
	            // 申请state=6标记为暂存
	            oaKqnotificationMag.saveObject(object);

	            object = oaKqnotificationMag.getObjectById(object.getDjId());
	    ...
	    public String saveAndSubmit() {
	        ...
	        // 保存
	        FUserDetail fuser = ((FUserDetail) getLoginUser());
	        OaKqnotification oaKqnotification = oaKqnotificationMag
	                .getObjectById(object.getDjId());
	        if (oaKqnotification != null) {
	            oaKqnotificationMag.copyObject(oaKqnotification, object);
	            object = oaKqnotification;
	        }
	        object.setCreater(fuser.getUsercode());
	        ...
	        if (object != null) {
	            ...
	            flowEngine.assignFlowOrganize(optBaseInfo.getFlowInstId(),
	                    "JSBM", vPowerUserInfo.getUnitcode(), "客情通报接收部门");
	            
	            this.saveIdeaInfo("客情通报登记", 0L);
	    ...
	    public String delete() {
	        if (StringUtils.isNotBlank(object.getDjId())) {
	            ...
	            oaKqnotificationMag.deleteObjectById(object.getDjId());
	        }
	        return "nextList";
	    }
	    ...
	    public String saveReditSQ() {
	        OaKqnotification oaKqnotification = oaKqnotificationMag
	                .getObjectById(object.getDjId());
	        if (oaKqnotification != null) {
	            oaKqnotificationMag.copyObjectNotNullProperty(
	                    oaKqnotification, object);
	            object = oaKqnotification;
	        }
	        oaKqnotificationMag.saveObject(object);
