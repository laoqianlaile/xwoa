...
public class OaLeaveapplyAction  extends OACommonBizAction<OaLeaveapply>  {
	private static final Log log = LogFactory.getLog(OaLeaveapplyAction.class);
	private static final long serialVersionUID = 1L;
	private OaLeaveapplyManager oaLeaveapplyMag;
	public void setOaLeaveapplyManager(OaLeaveapplyManager basemgr)
	{
		oaLeaveapplyMag = basemgr;
		this.setBaseEntityManager(oaLeaveapplyMag);
	}
    ...
    public String list() {
        ...
        objList = oaLeaveapplyMag.listObjects(filterMap, pageDesc);
    ...
    public String edit() {
        ...
        if (StringUtils.isBlank(object.getDjId())) {
            object.setDjId(optApplyManager.getNextDjId("SQ","oaLeaveapply"));
        }
   ...
   //                  否则  只保留上下文 /oa_szgh
    ...
    public String savePermitReg() {
        try {
            ...
            // 申请state=6标记为暂存
            oaLeaveapplyMag.saveObject(object);

            object = oaLeaveapplyMag.getObjectById(object.getDjId());
    ...
    public String saveAndSubmit() {
        ...
        // 保存
        FUserDetail fuser = ((FUserDetail) getLoginUser());
        OaLeaveapply oaLeaveapply = oaLeaveapplyMag
                .getObjectById(object.getDjId());
        if (oaLeaveapply != null) {
            oaLeaveapplyMag.copyObject(oaLeaveapply, object);
            object = oaLeaveapply;
        }
        object.setCreater(fuser.getUsercode());
         ...
         if("true".equals(SysParametersUtils.getParameters("generatePdfForDocEnable", "false"))){
           ...
           //"***/anonymous***"代表匿名访问
             ...
             taskExecutor.execute(new Runnable(){
                 @Override
                 public void run() {
                     optPdfInfoManager.createPDF(object.getDjId(), "请假条", 0L,userCode,exePath,formHtmlUrl);
                 }
             });
         }
     }
    ...
    public String delete() {
        if (StringUtils.isNotBlank(object.getDjId())) {
            ...
            oaLeaveapplyMag.deleteObjectById(object.getDjId());
        }
        return "nextList";
    }
    ...
    public String saveReditSQ() {
        OaLeaveapply oaLeaveapply = oaLeaveapplyMag
                .getObjectById(object.getDjId());
        if (oaLeaveapply != null) {
            oaLeaveapplyMag.copyObjectNotNullProperty(
                    oaLeaveapply, object);
            object = oaLeaveapply;
        }
        oaLeaveapplyMag.saveObject(object);
