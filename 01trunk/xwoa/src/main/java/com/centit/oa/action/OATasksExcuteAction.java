...
public class OATasksExcuteAction extends OACommonBizAction<OptProcInfo>
    ...
    public List<OptHtmlFrameInfo> getBaseInfoIframe(List<OptHtmlFrameInfo>  frameList,String djId){
        ...
        OptApplyInfo optApplyInfo = optApplyManager.getObjectById(optBaseInfo.getDjId());
        String moduleType =(optApplyInfo==null?"":optApplyInfo.getApplyItemType());
        ...
        if ("genOaApplyFrame".equals(this.initReturnFrame(djId))
            ...
            if(StringUtils.isNotBlank(moduleType)){
                if(checkCanEdit()){
                    frameList.add(OptApplyAction.getBizEditViewFrame(optBaseInfo.getDjId(),moduleType)); 
            }else{
                    frameList.add(OptApplyAction.getBizYWViewFrame(optBaseInfo.getDjId(),moduleType,"1"));    
                }
            }else{
                frameList.add(OptApplyAction.getBizDataViewFrame(djId));
            }
    ...
    public String doEditFlowOpt() {
        OptBaseInfo baseInfo = optBaseInfoManager.getOptBaseByFlowId(super
                .getFlowInstId());
        OptApplyInfo optApplyInfo=optApplyManager.getObjectById(baseInfo.getDjId());
        request.setAttribute("sqdjId", baseInfo.getDjId());
        List<OptHtmlFrameInfo> frameList = new ArrayList<OptHtmlFrameInfo>();

        this.setNodeInstId(object.getNodeInstId());
        String actionname = "optApply";
        String tableSpace="powerruntime";
       // if("SQ_BXDJ".equals(object.getNodeCode())||"SQ_DJ".equals(object.getNodeCode())){
            if(StringUtils.isNotBlank(optApplyInfo.getApplyItemType())){
                actionname=optApplyInfo.getApplyItemType();
                tableSpace="oa";
            }
        
            frameList.add(this.getReEditSQ(baseInfo.getDjId(),
                    object.getNodeCode(), object.getFlowPhase(),tableSpace,actionname,baseInfo.getPowerid()));
        // }
        this.boundAllInfoframe(frameList, baseInfo.getDjId(),
                this.initReturnFrame(baseInfo.getDjId()));
    ...
    private OptHtmlFrameInfo getReEditSQ(String djId, String nodeCode,
            String flowPhase,String tableSpace, String actionname, String itemId) {
        OptHtmlFrameInfo transFrameInfo = new OptHtmlFrameInfo();
        transFrameInfo.setFrameId("transFrame");
        transFrameInfo.setFrameSrc("/"+tableSpace+"/" +  actionname
                ...
                taskExecutor.execute(new Runnable(){
                    ...
                    public void run() {
                        ...
                        if(StringUtils.isBlank(formHtmlUrl)){
                             ...
                             formHtmlUrl = optPdfInfoManager.getSQFormHtmlUrl(contextPath,userCode, moduleType,object.getDjId());
                        }
