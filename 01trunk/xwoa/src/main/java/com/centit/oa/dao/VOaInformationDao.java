...
public class VOaInformationDao extends BaseDaoImpl<VOaInformation> {
    ...
    public Map<String, String> getFilterField() {
        if (filterField == null) {
            ...
            filterField.put("istop", CodeBook.EQUAL_HQL_ID);
            ...
            filterField.put("begcreatertime",
                    "creatertime >= to_date(?,'yyyy-MM-dd')");
            filterField.put("endcreatertime",
                    "creatertime <= to_date(?,'yyyy-MM-dd')");

            filterField.put("begReleaseDate",
                    "releaseDate >= to_date(?,'yyyy-MM-dd')");
            filterField.put("endReleaseDate",
                    "releaseDate <= to_date(?,'yyyy-MM-dd')");

            filterField
                    .put("currentdatetime",
                            "creatertime<=  to_timestamp(?,'yyyy-MM-dd hh24:mi:ss.ff')");

            filterField.put("info_Type", " info_Type = ? ");
            filterField.put("besides_info_Type", " info_Type != ? ");
            filterField
                    .put("NP_bettentime",
                            " to_date(to_char(releaseDate,'yyyy-MM-dd'),'yyyy-MM-dd')<=to_date(to_char(sysdate,'yyyy-MM-dd'),'yyyy-MM-dd') and ( validDate>=to_date(to_char(sysdate,'yyyy-MM-dd'),'yyyy-MM-dd') or validDate is null )");
            // 增加置顶排序
            filterField
                    .put(CodeBook.ORDER_BY_HQL_ID,
                            " (case when istop = 'T' and infoType = 'bulletin' then  11 when istop = 'T' and infoType != 'bulletin' then 12  else  99  end),creatertime desc");
        }
        return filterField;
    }
    ...
    public List<VOaInformation> listVoainformation(Map<String, Object> filterMap) {
        ...
        if (StringUtils.isNotBlank((String) filterMap.get("infoType"))) {
            infoType = " and info_Type="
                    + HQLUtils.buildHqlStringForSQL((String) filterMap
                            .get("infoType"));
        }
        if (StringUtils.isNotBlank((String) filterMap.get("NForbiddenState"))) {
            NForbiddenState = " and state is null or state ="
                    + HQLUtils.buildHqlStringForSQL((String) filterMap
                            .get("NForbiddenState"));
        }
        ...
        String sql = "select distinct NO,TITLE,RELEASE_DATE ,  "
                ...
                + shql + infoType + NForbiddenState + state + NP_bettentime
                + orderBy;
        List<Object[]> res = getHibernateTemplate().executeFind(
                new SQLQueryCallBack(sql));
        ...
        if (res != null && res.size() > 0) {
            for (Object[] oArr : res) {
                ...
                voaInfo.setCreatertime(oArr[2] != null ? DateUtil
                        .dateStr2DateTime(oArr[2].toString()) : null);
    ...
    @SuppressWarnings("unchecked")
    public List<VOaInformation> listUnReadInfo(Map<String, Object> filterMap,
            PageDesc pageDesc) {
        String sql = "select distinct t1.NO,t1.TITLE,t1.RELEASE_DATE,t1.STATE,t1.DOC_NO,t1.creater,t1.istop,t1.INFO_TYPE,t1.creatertime from v_oa_information t1 "
                + "   left join F_USERBIZOPT_LOG t2  on t1.no = t2.dj_id and t2.createuser="
                + HQLUtils.buildHqlStringForSQL(filterMap.get("creater")
                        .toString()) + "   where t2.dj_id is null";
        if (filterMap.get("state") != null) {
            sql += " and t1.state = "
                    + HQLUtils.buildHqlStringForSQL(filterMap.get("state")
                            .toString());
        }

        sql += " order by (case when t1.istop = 'T' and t1.INFO_TYPE = 'bulletin' then  11 when t1.istop = 'T' and t1.INFO_TYPE != 'bulletin' then 12  else  99  end),t1.creatertime desc";
        List<Object[]> res = (List<Object[]>) super.findObjectsBySql(sql,
        ...
        if (res != null && res.size() > 0) {
            for (Object[] oArr : res) {
                ...
                voaInfo.setNo(oArr[0] != null ? oArr[0].toString() : "");
                voaInfo.setTitle(oArr[1] != null ? oArr[1].toString() : "");
                voaInfo.setReleaseDate(oArr[2] != null ? DateUtil
                        .dateStr2DateTime(oArr[2].toString()) : null);
                voaInfo.setState(oArr[3] != null ? oArr[3].toString() : "");
                voaInfo.setDocNo(oArr[4] != null ? oArr[4].toString() : "");
                voaInfo.setCreater(oArr[5] != null ? oArr[5].toString() : "");
                voaInfo.setIsTop(oArr[6] != null ? oArr[6].toString() : "");
                voaInfo.setInfoType(oArr[7] != null ? oArr[7].toString() : "");
                voaInfo.setCreatertime(oArr[8] != null ? DateUtil
                        .dateStr2DateTime(oArr[8].toString()) : null);
                voaInfo.setReadstate("未读");
                voaInfoList.add(voaInfo);
            }
        }

        List<BigDecimal> list = (List<BigDecimal>) this
                .findObjectsBySql("SELECT COUNT(*) from ("
                        + SQLUtils.removeOrderBy(sql) + ")");

        if (null != list && !list.isEmpty()) {
            pageDesc.setTotalRows(Integer.valueOf(list.get(0).toString()));
        }
        return voaInfoList;
    }
