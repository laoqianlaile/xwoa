...
public class OaLeavereportedAction  extends OACommonBizAction<OaLeavereported>  {
	private static final Log log = LogFactory.getLog(OaLeavereportedAction.class);
	private static final long serialVersionUID = 1L;
	private OaLeavereportedManager oaLeavereportedMag;
	public void setOaLeavereportedManager(OaLeavereportedManager basemgr)
	{
		oaLeavereportedMag = basemgr;
		this.setBaseEntityManager(oaLeavereportedMag);
	}
     ...
     public String list() {
         ...
         objList = oaLeavereportedMag.listObjects(filterMap, pageDesc);
     ...
     public String edit() {
         ...
         if (StringUtils.isBlank(object.getDjId())) {
             object.setDjId(optApplyManager.getNextDjId("SQ","oaLeavereported"));
         }
         if(StringUtils.isBlank(object.getCreater())){
          // 新添加申请时，默认将系统登录人作为申请人
             ...
             if(StringUtils.isBlank(object.getCreater())){
                 ...
                 object.setSolvenote(user.getUsername());
             }
             if(null==object.getCreatertime()){
                 object.setCreatertime(new Date());
             }
             if(StringUtils.isBlank(object.getPostleve())){
                 object.setApplyuser(user.getPrimaryUnit());
                 object.setPostleve(user.getUserdesc());
             }
         }
    ...
    //                  否则  只保留上下文 /oa_szgh
     ...
     public String savePermitReg() {
         try {
             ...
             // 申请state=6标记为暂存
             oaLeavereportedMag.saveObject(object);

             object = oaLeavereportedMag.getObjectById(object.getDjId());
     ...
     public String saveAndSubmit() {
         ...
         // 保存
         FUserDetail fuser = ((FUserDetail) getLoginUser());
         OaLeavereported oaLeavereported = oaLeavereportedMag
                 .getObjectById(object.getDjId());
         if (oaLeavereported != null) {
             oaLeavereportedMag.copyObject(oaLeavereported, object);
             object = oaLeavereported;
         }
         //object.setCreater(fuser.getUsercode());
         ...
         object.setCreatertime(DatetimeOpt.currentUtilDate());
         object.setCreater(fuser.getUsercode());
         object.setTransaffairname(object.getSolvenote() + "外出请假报备单");
         // 申请isuse状态为1
         ...
         if (object != null) {
             ...
             //将选择分管领导纳入办件角色
        /*     String [] teamUsers=super.teamUserCodes==null?null:super.teamUserCodes.split(",");
             if(teamUsers!=null){
                 for(String teamUser:teamUsers){
                     flowEngine.assignFlowWorkTeam(flowInstId,super.teamRoleCode,
                             teamUser, "分管领导审核");
                     }
                 } */          
             // 将登记部门纳入权限引擎
             flowEngine.assignFlowOrganize(optBaseInfo.getFlowInstId(), "DJBM",
                     fuser.getPrimaryUnit(), "外出请假报备登记");
          ...
          if("true".equals(SysParametersUtils.getParameters("generatePdfForDocEnable", "false"))){
            ...
            //"***/anonymous***"代表匿名访问
              ...
              taskExecutor.execute(new Runnable(){
                  @Override
                  public void run() {
                      optPdfInfoManager.createPDF(object.getDjId(), "外出请假报备", 0L,userCode,exePath,formHtmlUrl);
                  }
              });
          }
      }
     ...
     public String delete() {
         if (StringUtils.isNotBlank(object.getDjId())) {
             ...
             oaLeavereportedMag.deleteObjectById(object.getDjId());
         }
         return "nextList";
     }
     ...
     public String saveReditSQ() {
         OaLeavereported oaLeavereported = oaLeavereportedMag
                 .getObjectById(object.getDjId());
         if (oaLeavereported != null) {
             oaLeavereportedMag.copyObjectNotNullProperty(
                     oaLeavereported, object);
             object = oaLeavereported;
         }
         oaLeavereportedMag.saveObject(object);
