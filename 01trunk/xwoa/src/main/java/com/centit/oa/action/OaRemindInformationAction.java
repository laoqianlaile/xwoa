...
BaseEntityExtremeAction<OaRemindInformation> {
   ...
   public String builtV2(){
    ...
    public void sendMsg(String receptcode){
        ...
        if(StringUtils.isNotBlank(isSendMsg) && "T".equals(isSendMsg) && StringUtils.isNotBlank(receptcode)){
            ...
            remindContent = StringUtils.replace(remindContent, "{username}", CodeRepositoryUtil.getValue("usercode", receptcode));
    ...
    public void submitForDUBAN() {
        try {
            ...
            //reType  事权类型处理--以SQ开头，主要排除CARSQ 等
            if(object.getReType().startsWith("SQ")){
                object.setReType("SQ");
            }
