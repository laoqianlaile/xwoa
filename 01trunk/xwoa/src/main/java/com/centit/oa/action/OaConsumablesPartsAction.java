...
public class OaConsumablesPartsAction  extends OACommonBizAction<OaConsumablesParts>  {
	private static final Log log = LogFactory.getLog(OaConsumablesPartsAction.class);
	private static final long serialVersionUID = 1L;
	private OaConsumablesPartsManager oaConsumablesPartsMag;
	public void setOaConsumablesPartsManager(OaConsumablesPartsManager basemgr)
	{
		oaConsumablesPartsMag = basemgr;
		this.setBaseEntityManager(oaConsumablesPartsMag);
	}
	 ...
	 private VsuppowerinusingManager vsuppowerinusingManager;
	    ...
	    public String list() {
	        ...
	        objList = oaConsumablesPartsMag.listObjects(filterMap, pageDesc);
	    ...
	    public String edit() {
	        ...
	        if (StringUtils.isBlank(object.getDjId())) {
	            object.setDjId(optApplyManager.getNextDjId("SQ","oaConsumablesParts"));
	        }
	        if(StringUtils.isBlank(object.getCreater())){
	         // 新添加申请时，默认将系统登录人作为申请人
	            ...
	            if(StringUtils.isBlank(object.getUnitcode())){
	                object.setUnitcode(user.getPrimaryUnit());
	            }
	        }
	   ...
	   //                  否则  只保留上下文 /oa_szgh
	    ...
	    public String savePermitReg() {
	        try {
	            ...
	            // 申请state=6标记为暂存
	            oaConsumablesPartsMag.saveObject(object);

	            object = oaConsumablesPartsMag.getObjectById(object.getDjId());
	    ...
	    public String saveAndSubmit() {
	        ...
	        // 保存
	        FUserDetail fuser = ((FUserDetail) getLoginUser());
	        OaConsumablesParts oaConsumablesParts = oaConsumablesPartsMag
	                .getObjectById(object.getDjId());
	        if (oaConsumablesParts != null) {
	            oaConsumablesPartsMag.copyObject(oaConsumablesParts, object);
	            object = oaConsumablesParts;
	        }
	        object.setCreater(fuser.getUsercode());
	        ...
	        if (object != null) {
	            ...
	            flowEngine.assignFlowOrganize(optBaseInfo.getFlowInstId(), "DJBM",
	                    fuser.getPrimaryUnit(), "耗材配件购置信息登记部门");
                ...
                flowEngine.assignFlowOrganize(optBaseInfo.getFlowInstId(),
                        "JSBM", vPowerUserInfo.getUnitcode(), "耗材配件购置信息接收部门");
                
	            this.saveIdeaInfo("耗材配件购置信息登记", 0L);
	        }
	         ...
	         if("true".equals(SysParametersUtils.getParameters("generatePdfForDocEnable", "false"))){
	           ...
	           //"***/anonymous***"代表匿名访问
	             ...
	             taskExecutor.execute(new Runnable(){
	                 @Override
	                 public void run() {
	                     optPdfInfoManager.createPDF(object.getDjId(), "耗材配件购置信息", 0L,userCode,exePath,formHtmlUrl);
	                 }
	             });
	         }
	     }
	    ...
	    public String delete() {
	        if (StringUtils.isNotBlank(object.getDjId())) {
	            ...
	            oaConsumablesPartsMag.deleteObjectById(object.getDjId());
	        }
	        return "nextList";
	    }
	    ...
	    public String saveReditSQ() {
	        OaConsumablesParts oaConsumablesParts = oaConsumablesPartsMag
	                .getObjectById(object.getDjId());
	        if (oaConsumablesParts != null) {
	            oaConsumablesPartsMag.copyObjectNotNullProperty(
	                    oaConsumablesParts, object);
	            object = oaConsumablesParts;
	        }
	        oaConsumablesPartsMag.saveObject(object);
