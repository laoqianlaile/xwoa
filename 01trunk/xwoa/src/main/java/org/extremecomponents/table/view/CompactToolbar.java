...
/**
 * 为了兼容现有的样式，将修改columnRight方法 基于1.1.3版本源码
 * @since1.1.3
...
public class CompactToolbar extends TwoColumnRowLayout {
    ...
    protected void columnRight(HtmlBuilder html, TableModel model) {
        ...
        html.td(4).styleClass(BuilderConstants.STATUS_BAR_CSS).align("right").close();
    ...
    protected void columnLeft(HtmlBuilder html, TableModel model) {
        ...
        html.table(4).border("0").cellPadding("1").cellSpacing("0").close();
        html.tr(5).close();

        if (showPagination) {
            //每页多少条
            html.td(5).width("100").close();
            toolbarBuilder.rowsDisplayedDroplist();
            html.tdEnd();
            //分割符
            html.td(5).width("10").close();
            toolbarBuilder.separator();
            html.tdEnd();
            //首页
            html.td(5).width("30").close();
