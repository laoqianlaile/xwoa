...
public class DataCatalogDao extends BaseDaoImpl<FDatacatalog> {
	...
	public Map<String, String> getFilterField() {
		if( filterField == null){
			...
			filterField.put("CATALOGSTYLENOT" , "catalogstyle !=?");
			filterField.put("DATAVALUE" , "catalogcode in (select id.catalogcode from FDatadictionary where datavalue like ?)");
			filterField.put("DICTIONARYTYPE" , CodeBook.EQUAL_HQL_ID);
		}
		return filterField;
	}	
}
