...
public class OACommonBizAction<T> extends PowerRuntimeEntityAction<T> implements
    ...
    // add by hx 权力登记与人员关联
    protected VPowerUserInfo vPowerUserInfo;
    protected VPowerUserInfoManager vPowerUserInfoManager;
    ...
    public String generalOpt() {
        try {
            ...
            if ("T".equals(moduleParam.getHasOrgRole())) {
                ...
                if(StringUtils.isNotBlank(nodeCode) && !"hys_ap".equals(nodeCode)){
                initPFUnit(moduleParam.getXbOrgRoleCode(), nodeCode);
                }else if("hys_ap".equals(nodeCode) || StringUtils.isBlank(nodeCode)){//确定会议保障部门
                    initAPUnit(moduleParam.getXbOrgRoleFilter(), nodeCode); 
                }
                initXbOrgConfig();
            }
...
//            int strSize = (int) str.length();
     ...
     */
     ...
     /**
      * 根据事项配置初始化通用配置（流程属性配置）
      * @param itemId
      */
    public void initSQModuleDataByItemId(String itemId) {
     // 获取事权对应的信息
