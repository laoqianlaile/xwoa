...
public class OaInformationAction extends BaseEntityDwzAction<OaInformation> {
    ...
    public String view(){
        ...
        UserbizoptLog u=new UserbizoptLog(new UserbizoptLogId(o.getTitle(), o.getNo()));
        ...
        List<OaInformationAttachment> videoAttachments = oaInformationAttachmentManager.findVideoAttachments(o.getNo());
    ...
    public String list() {
        try {
            ...
            //默认查询当前月份第一天到现在的记录
            if(StringUtils.isNotBlank((String)filterMap.get("begReleaseDate"))&&StringUtils.isNotBlank((String)filterMap.get("endReleaseDate"))){
                /*filterMap.put("begReleaseDate",DateUtil.getCurrentMonthOfDay() );
                filterMap.put("endReleaseDate", DateUtil.convertDateToString(new Date(), "yyyy-MM-dd"));
                request.setAttribute("s_begReleaseDate",DateUtil.getCurrentMonthOfDay() );
                request.setAttribute("s_endReleaseDate",DatetimeOpt.convertDateToString(new Date(), "yyyy-MM-dd") );*/
                filterMap.put("endReleaseDate", DateUtil.convertDateToString(DatetimeOpt.convertStringToDate((String)filterMap.get("endReleaseDate"),"yyyy-MM-dd"), "yyyy-MM-dd"));
                request.setAttribute("endReleaseDate", DateUtil.convertPrevDateToString(DatetimeOpt.convertStringToDate((String)filterMap.get("endReleaseDate"),"yyyy-MM-dd"), "yyyy-MM-dd"));
            }/*else{
            ...
            if(vobjList!=null&&vobjList.size()>0){
                for(int i=0;i<vobjList.size();i++){
                    ...
                    //=============办件添加是否阅读状态start=============
                    UserbizoptLog uboptlog=userbizoptLogManager.listObject(oa.getNo(), userCode);
                    if(uboptlog!=null){
    ...
    public String oaInfomationMainpage(){
        ...
        //图片新闻列表
        objList=(List<OaInformation>)getSubList(baseEntityManager.listObjects(filterMap, pageDesc),5);
        ...
        if(null!=infoTypesList){
            ...
            for(FDatadictionary f:infoTypesList){
              ...
              //列表不为空时截取固定长度
                List<OaInformation> info = (List<OaInformation>)getSubList(baseEntityManager.listObjects(filterMap, pageDesc),9);
                infoList.add(info);
        ...
        try {
            ...
            if(video!=null){
                for(int i=0;i<video.length;i++){
                    EfileStore efileStore = new EfileStore(video[i], videoFileName[i],  dirPath);
                    EfileManager.store(efileStore);
