...
public class VSwSearchDao extends BaseDaoImpl<VSwSearch> {
    private static final long serialVersionUID = 1L;
    public static final Log log = LogFactory.getLog(VSwSearchDao.class);
    ...
    public Map<String, String> getFilterField() {
        if (filterField == null) {
            ...
            filterField.put("begNo", "to_number(substr(acceptArchiveNo,7,4)) >= ? ");
            filterField.put("endNo", "to_number(substr(acceptArchiveNo,7,4)) <= ? ");
            
            filterField.put("swhTime1", "incomeDate >= to_date(?,'yyyy-mm-dd')");
            filterField.put("swhTime2", "incomeDate <= to_date(?,'yyyy-mm-dd')");
            filterField.put("pfTime1", "transdate >= to_date(?,'yyyy-mm-dd')");
            filterField.put("pfTime2", "transdate <= to_date(?,'yyyy-mm-dd')");
