		...
		<s:form action="supPower" namespace="/powerbase"	style="margin-top:0;margin-bottom:5">
			...
			<input type="hidden" name="itemType" value="${itemType}">	
				...
				<tr>
					...
					<s:submit method="listSupPower_CF_faci" key="opt.btn.query" cssClass="btn" /> 
	...
	<ec:table action="powerbase/suppower!listSupPower_CF_faci.do" items="selectPowerList" var="suppower"
		...
		<ec:row>
			...
			<ec:column property="opt" title="操作" sortable="false"
				...
				<%-- <input type="radio" name="itemId" onclick="setParentVal('${suppower.itemId}')"> --%>
				<input type="radio" onclick="selectCF4XCCF('${suppower.itemId}')">
			</ec:column>
		</ec:row>
	</ec:table>
...
<script type="text/javascript">	
	...
	function selectCF4XCCF(itemId) {
		if (window.confirm("确认选择此处罚项目吗？选择后窗口将自动关闭。")) {
			var url ="<%=request.getContextPath()%>/punish/punishobjectbasic!createfacilitydes.do?item_id=" + itemId +"&object.punishobjectid=${punishobjectid}";
