...
<body class="sub-flow">
	...
	<table cellpadding="0" cellspacing="0" >
		...
		<tr>
			<td>&nbsp;</td>
			<td colspan="5">
			 <input type="button" value="添加用户机构" class="btn btnWide" onclick="location.href='userDef!addUserUnderUnit.do?usercode=${usercode}'" />
			 <input type="button" value="添加用户角色" class="btn btnWide" onclick="location.href='deptManager!bulitUserRole.do?userrole.usercode=${usercode}'" />
			 <%-- <a href="userDef!addUserUnderUnit.do?usercode=${usercode}" ><span class="btn btnWide">添加用户机构</span></a>	 		
			 <a href="deptManager!bulitUserRole.do?userrole.usercode=${usercode}"><span class="btn">添加用户角色</span></a> --%>
			<input type="button" value="返回" class="btn" onclick="location.href='userDef!listUserInfo.do?s_byUnderUnit=thisunit&ec_p=${param.ec_p}&ec_crd=${param.ec_crd}'">					
			</td>			
		</tr>
	</table>
     ...
     <ec:table items="userunits" var="fUserunit"  action="userDef!view.do?usercode=${usercode}" sortable="false" showPagination="false"
		...
		<ec:row>
			...
			<ec:column property="unitopt" title="操作" sortable="false"	 >
				...
				<a href='userDef!editUserUnit.do?userUnit.usercode=${usercode}&userUnit.unitcode=${fUserunit.unitcode}&userUnit.userstation=${fUserunit.userstation}&userUnit.userrank=${fUserunit.userrank}&underUnit=T'>
					编辑
				</a>
				<a href='userDef!deleteUserUnit.do?userUnit.usercode=${usercode}&userUnit.unitcode=${fUserunit.unitcode}&userUnit.userstation=${fUserunit.userstation}&userUnit.userrank=${fUserunit.userrank}&underUnit=T'
					onclick='return confirm("是否删除用户机构：${cp:MAPVALUE("unitcode",fUserunit.unitcode)}?");'>
					删除
				</a>
				</c:if>
			</ec:column>
...
%>
      ...
      <ec:table items="userroles" var="role" action="userDef!view.do?usercode=${userinfo.usercode}" sortable="false"  showPagination="false"
			...
			<ec:row>
				...
				<ec:column property="obtaindate" title="获取时间" style="text-align:center"/>
				...
				<ec:column property="roleopt" title="操作" sortable="false"	 >
					...
					<a href='userDef!editUserRole.do?usercode=${usercode}&userrole.id.rolecode=${role.rolecode}&obtaindate=${role.obtaindate}&underUnit=T'>
						编辑
					</a>
					
						<a href='userDef!deleteUserRole.do?userrole.usercode=${usercode}&userrole.rolecode=${role.rolecode}&userrole.obtaindate=${role.obtaindate}&underUnit=T'
