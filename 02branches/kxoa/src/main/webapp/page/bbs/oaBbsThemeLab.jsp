			...
			var param = {
				...
				callback : {
					onClick : function(eventjs, treeId, treeNode) {
					 if("unit"==treeNode.choosetype){
//					 $("#tabFrames1").attr("src",'<%=request.getContextPath()%>/bbs/oaBbsDashboard!view.do?layoutcode='+ treeNode.id+ '&s_discussOrNot=Y&s_isOwner=T' ); --%>
                         $("#tabFrames1").attr("src",'<%=request.getContextPath()%>/bbs/vBbsThemeUser!list.do?s_layoutcode='+ treeNode.id);
						}else if("dis"==treeNode.choosetype){
						  $("#tabFrames1").attr("src",'<%=request.getContextPath()%>/bbs/vBbsThemeUser!list.do?s_layoutno='+ treeNode.id);
					
						}else{
							//顶级
							$("#tabFrames1").attr("src",'<%=request.getContextPath()%>/bbs/vBbsThemeUser!list.do');
							}	
					}
				}
			};
