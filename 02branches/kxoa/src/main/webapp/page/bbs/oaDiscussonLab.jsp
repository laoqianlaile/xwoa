			...
			var param = {
				...
				callback : {
					onClick : function(eventjs, treeId, treeNode) {
                             $("#tabFrames1").attr("src",'<%=request.getContextPath()%>/bbs/oaBbsDiscussion!list.do?s_isOwner=T&s_discussOrNot=Y&s_layoutcode='+ treeNode.id);
...
// 								+ treeNode.id + '&type=' + treeNode.p;
			...
			if (nodes.length>0) {
				...
				$("#tabFrames1").attr("src",'<%=request.getContextPath()%>/bbs/oaBbsDiscussion!list.do?s_isOwner=T&s_discussOrNot=Y&s_layoutcode=0');
			}
