...
<s:form action="twodimenform">
				...
				<table cellpadding="0" cellspacing="0" align="center" class="tablegray">
					...
					<td>是否合并&nbsp; <select id="selectunit" name="modelName">
							...
							<option
								<c:if test="${modelName=='xzqlsltjhb'}">selected="selected"</c:if>
								value="xzqlsltjhb">是</option>
							<option
								<c:if test="${modelName=='xzqlsltj'}">selected="selected"</c:if>
								value="xzqlsltj">否</option>
			...
			<c:forEach var="col" items="${columns}" varStatus="status">
				...
				<c:if test="${rowInd > 2}">
					...
					<ec:column property="${col.colProperty}" title="${col.colName}"
						...
						<c:if test="${modelName=='xzqlsltjhb'}">
						 ${optinfo[name] }
						</c:if>
						
						<c:if test="${modelName=='xzqlsltj'}">
