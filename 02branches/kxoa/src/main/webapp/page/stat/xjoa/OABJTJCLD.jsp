...
<%@ include file="/page/stat/childs/stat-taglibs.jsp"%>
<!-- IFRAME模式下需要导入的内容END -->
...
$(function() {
	       //格式化合计数据格式
	        $('#statTable>tbody>tr>td:nth-child(18)').each(function() {
	        	$(this).text(parseNumber($(this).text().replace(/[‘&\|\\\*^%$#@\-“]/g,"").trim()));
	        });   
         ...
         for(var n=0;n<8;n++){
     	 ...
     	 for(var i=0;i<8;i++){  
     	     ...
     	     $('#statTable>tbody>tr:nth-child('+target+') td').eq(2*i+1).each(function() {
     	    	 bj.push({y:parseNumber($(this).text().replace(/[‘&\|\\\*^%$#@\-“]/g,"").trim()),link:$(this).find("a").attr('href')});
//      	    	 bjurl.push($(this).html());
     	     });   
     	    }
     	    for(var j=0;j<8;j++){  	
     	     $('#statTable>tbody>tr:nth-child('+target+') td').eq(2*j+2).each(function() {
     	    	 bl.push({y:parseNumber($(this).text().replace(/[‘&\|\\\*^%$#@\-“]/g,"").trim()),link:$(this).find("a").attr('href')});
