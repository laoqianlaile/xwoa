...
<body>
	...
	<ec:tree identifier="col0" parentAttribute="col1"
		...
		<c:forEach var="col" items="${columns}" varStatus="status">
				...
				<c:if test="${rowInd > 2}">
					<ec:column property="${col.colProperty}" title="${yearN}${(monthN+status.index-3+1)>12?(monthN+status.index-3+1-12):(monthN+status.index-3+1)}月" sortable="false" style="text-align:left">
