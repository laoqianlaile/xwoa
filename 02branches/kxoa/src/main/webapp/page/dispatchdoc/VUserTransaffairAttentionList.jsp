...
<html>
	<head>
		<title>
			<s:text name="VUserTransaffairAttention.list.title" />
		</title>
	...
	<body>
		...
		<fieldset>
			...
			<s:form action="VUserTransaffairAttention" namespace="/dispatchdoc" style="margin-top:0;margin-bottom:5">
...
<%-- 							 yearRange="2000:2020" displayFormat="yy-mm-dd" changeYear="true" /> --%>
		...
		<ec:table action="dispatchdoc/VUserTransaffairAttention!list.do" items="objList" var="VUserTransaffairAttention"
			...
			<ec:row>
				...
				<a href='<%=request.getContextPath()%>/dispatchdoc/ioDocTasksExcute!transaffairView.do?djId=${VUserTransaffairAttention.djId}&flowInstId=${VUserTransaffairAttention.flowInstId}'>${VUserTransaffairAttention.transaffairname}</a>
				</ec:column>
				...
				<ec:column property="headunitcode" title="主办处室" style="text-align:left">
					${cp:MAPVALUE("unitcode",VUserTransaffairAttention.headunitcode)}
				</ec:column>
				<ec:column property="headusercode" title="主办承办人" style="text-align:left">
					${cp:MAPVALUE("usercode",VUserTransaffairAttention.headusercode)}
				</ec:column>
				...
				<ec:column property="opt" title="${optlabel}" sortable="false"
					...
					<a href="#" onclick="viewFlow('${VUserTransaffairAttention.flowInstId}');">查看流程图</a>
				</ec:column>
