...
<body class="sub-flow">
	...
	<s:form action="incomeDoc" method="post" namespace="/dispatchdoc"
		id="incomeDocForm">
		...
		<input type="hidden" id="incomeDocType" name="incomeDocType"
			value="SWXZ" />
		<fieldset class="_new">
			<legend>阅办文登记</legend>
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0"
				...
				<input type="hidden" id="itemId" name="itemId"
					value="${object.optBaseInfo.powerid}">
				<tr>
					<td class="addTd">来文标题<font color="#ff0000">*</font>
					</td>
					<td align="left" colspan="3"><input type="text"
						id="incomeDocTitle" name="incomeDocTitle" maxlength="200"
						value="${object.incomeDocTitle}"
				...
				<tr>
					...
					<td align="left" style="position: relative; z-index: 100;"><input
						type="text" id="incomeDeptName" name="incomeDeptName"
						maxlength="200" value="${object.incomeDeptName}"
						style="width: 100%; height: 25px;"
						onkeyup="incomeDeptAuto.handleEvent(this.value,'incomeDeptName',event)" />
						<div id="deptDivc" class="divc"></div></td>
					<td class="addTd">收文<span
						style="font-family: Simsun !important">字号</span> <!-- 								<font color="#ff0000">*</font> -->
					</td>
					<td align="left"><input type="text" id="incomeDocNo"
						name="incomeDocNo" maxlength="50" value="${object.incomeDocNo}"
						style="width: 100%; height: 25px;" /></td>
				...
				<%-- <tr>
						 <td class="addTd">
							...
							<td align="left" colspan="3">
								<select id="incomeDeptType" name="incomeDeptType" style="width: 200px;height:25px;">
								
								<c:if test="${empty object.incomeDocType }">
							     <c:forEach var="row" items="${cp:DICTIONARY('SWXZ')}">
							     <option value="${row.key}" ${(object.incomeDeptType eq row.key or (empty object.incomeDeptType and row.key eq '1')) ? 'selected = "selected"' : ''}> 
										<c:out value="${row.value}" />
										</option>
							    </c:forEach>
								</c:if>
								<c:if test="${not empty object.incomeDocType }">
								 <c:forEach var="row" items="${cp:DICTIONARY(object.incomeDeptType)}">
								  <option value="${row.key}" ${(object.incomeDeptType eq row.key or (empty object.incomeDeptType and row.key eq '1')) ? 'selected = "selected"' : ''}>
											<c:out value="${row.value}" />
										</option>
							   </c:forEach>
				...
				<tr>
					...
					<td align="left" style="position: relative">
							...
							<input type="hidden" id="acceptArchiveNo"
						name="optBaseInfo.acceptArchiveNo"
						value="${object.optBaseInfo.acceptArchiveNo}" /> <%-- <span id="acceptArchiveNoTitle"></span>
							<input type="text" id="acceptArchiveNoNum" 
							name="acceptArchiveNoNum" style="width:8%" maxlength="100"
							onblur="acceptArchiveNoMouseout();" 
							value="${object.optBaseInfo.acceptArchiveNo}"  />	 --%> <span
						id="acceptArchiveNoTail"></span> <input type="text"
						name="acceptArchiveNoPrex" id="acceptArchiveNoPrex" placeholder="请输入来文单位分类"
						onblur="acceptArchiveNoMouseout();"
						onkeyup="incomeAcceptAuto.handleEvent(this.value,'acceptArchiveNoPrex',event)" />
						<div id="acceptDivc" class="divc"></div> <span
						id="acceptArchiveNoYear"></span> <input type="text"
						id="acceptArchiveNumber" name="acceptArchiveNumber"
						placeholder="请输入编号" onblur="acceptArchiveNoMouseout();" /> <span
						id="acceptArchiveNoSuffix">号</span>
					</td>
					<td class="addTd">收文日期<font color="#ff0000">*</font>
					</td>
					<td align="left"><input type="text" class="Wdate"
						...
						onpropertychange="changeSwh();" placeholder="选择日期"></td>
				</tr>
				<tr>
					<td class="addTd">缓急程度</td>
					<td align="left"><select id="criticalLevel"
						name="criticalLevel" style="width: 200px; height: 27px">
							<option value="">---请选择---</option>
							<c:forEach var="row" items="${cp:DICTIONARY('critical_level')}">
								<option value="${row.key}"
									${(object.criticalLevel eq row.key or (empty object.criticalLevel and row.key eq '0')) ? 'selected = "selected"' : ''}>
									<c:out value="${row.value}" />
								</option>
							</c:forEach>
					</select></td>
					...
					<td align="left" style="position: relative">
						<select id="incomeDeptType" name="incomeDeptType" style="width: 200px;height:27px">
							<option value="">---请选择来文分类单位---</option>
				...
				<c:if
					test="${'T' eq cp:MAPVALUE('sendMSg','isopen') and 'T' eq moduleParam.canSendMessage }">
					<tr>
...
<script
	src="${pageContext.request.contextPath}/newStatic/js/custom/common/autoSelect.js"
	type="text/javascript"></script>
<script type="text/javascript">
	...
	$()
			.ready(
					function() {
						...
						showDepType(val);
						//设置选中项
						if ('${object.incomeDeptType}' != null
								&& '${object.incomeDeptType}' != '') {
							setTimeout(
									function() {
						...
						if (null == $('#acceptArchiveNoYear').text()
								|| '' == $('#acceptArchiveNoYear').text()) {
							$('#acceptArchiveNoYear').html(
	...
	function showDepType(val) {
		var refUrl = "${pageContext.request.contextPath}/dispatchdoc/incomeDoc!incomeDoc.do?datacode="
				+ val;
	...
	$("#incomeDeptType").click(
			function() {
				...
				//不同字典项相同名字的部门类型值也不一样
				if (incomeDeptType == '${object.incomeDeptType}'
						&& swdate == '${object.incomeDate}') {
					$("#acceptArchiveNo").val(
	...
	function changeSwh() {
		...
		$
				.ajax({
					type : "POST",
					url : "${contextPath}/dispatchdoc/incomeDoc!changeSwh.do?incomeDocType="
							+ incomeDocType
							+ "&incomeDeptType="
							+ incomeDeptType + "&swdate=" + swdate,
					dataType : "json",
	...
	function editShip(djId, no) {
		JDialog.open({
			src : "${contextPath}/dispatchdoc/incomeDoc!editShip.do?djId="
					+ djId + "&ship.no=" + (no ? no : ""),
			width : 800,
			height : 350
		});
	...
	function deleteShip(self, djId, no) {
		var result = $.ajax({
			url : "${contextPath}/dispatchdoc/incomeDoc!deleteShip.do?djId="
					+ djId + "&ship.no=" + no,
			async : false
		}).responseText;
		if ("success" == result) {
	...
	function doSubmitCheck(subType) {
		...
		/* if ("" == $.trim($("#acceptArchiveNo").val())) {
			alert("阅办文编号不可为空！");
			$('#acceptArchiveNo').focus();
			return false;
		} */

		/* if ("" == $.trim($("#acceptArchiveNoNum").val())) {
			alert("阅办文编号不可为空！");
			$('#acceptArchiveNo').focus();
			return false;
		} */
		...
		/* if ("" == $.trim($("#acceptArchiveNoPrex").val())) {
			alert("来文单位分类不可为空！");
			$('#acceptArchiveNoPrex').focus();
			return false;
		} */

		/* if ("" == $.trim($("#acceptArchiveNumber").val())) {
			alert("编号不可为空！");
			$('#acceptArchiveNumber').focus();
			return false;
		} */

		if ("" != $.trim($("#acceptArchiveNoPrex").val())
				|| "" != $.trim($("#acceptArchiveNumber").val())) {
			/* if ("" == $.trim($("#acceptArchiveNoPrex").val())) {
				alert("来文单位不可为空！");
				$('#acceptArchiveNoPrex').focus();
				return false;
		...
		/* if ("" == $.trim($("#incomeDeptType").val())) {
			alert("来文单位分类不可为空！");
			$('#incomeDeptType').focus();
			return false;
		} */

		/* if ("" == $.trim($("#secretDegree").val())) {
			alert("密级不可为空！");
			$('#secretDegree').focus();
			return false;
		} */
		...
		/* if ("" == $.trim($("#toBeanfinishedDate").val())) {
			alert("办结截止日期不可为空！");
			$('#toBeanfinishedDate').focus();
			return false;
		} */
	...
	function refreshShip(jsonArray) {
		...
		if (jsonArray && jsonArray.length > 0) {
			for (var i = 0; i < jsonArray.length; i++) {
				...
				buffer.append("<tr><td>" + shipObj.leaderName + "</td><td>"
						+ shipObj.unitName
		...
		if (acceptArchiveNo != null && acceptArchiveNo != '') {
			...
			$
					.ajax({
						...
						data : {
							"djId" : djId,
							"acceptArchiveNo" : acceptArchiveNo
						},
						dataType : "json",
						success : function(json) {
							if (json.msg == "F") {
								alert("该阅办文编号已存在，请确认后再次输入！");
							} else if (json.msg == "T") {
								t = true;
							}
						},
	...
	function split() {
		...
		if (s.length > 1) {
			var title = s[0] + "〕";
			var tail = acceptArchiveNo.substring(acceptArchiveNo.length - 1,
					acceptArchiveNo.length);
	...
	function splitV2() {
		...
		if (array1.length > 1) {
			...
			if (array2.length > 1) {
				...
				$('#acceptArchiveNumber').val(
						array2[1].substring(0, array2[1].length - 1));
			}
		}
		acceptArchiveNoMouseout();
	}
	...
	function spliceV2() {
		...
		$("#acceptArchiveNo")
				.val($.trim(prex) + year + $.trim(number) + suffix);
	}
