...
<body>
	...
	<fieldset class="search">
		<legend class="headTitle">
			<c:choose>
				<c:when test="${show_type eq 'T' or s_applyItemType eq 'GW' }">阅办文查看</c:when>
				<c:when test="${empty show_type  or empty  s_applyItemType}">个人阅办文</c:when>
			</c:choose>
		</legend>
		<div class="searchDiv">
			<s:form id="incomedoclistv1form" action="incomeDoc"
				namespace="/dispatchdoc" method="post"
				style="margin-top:0;margin-bottom:5">
				<div class="searchArea">
					<s:hidden name="show_type" value="%{show_type}" />
					<input type="hidden" name="s_applyItemType"
						value="${s_applyItemType}" />
					<table style="width: auto;">
						<tr>
							<td class="searchBtnArea"><s:submit method="exportExcelByPo"
									value="导出Excel" cssClass="whiteBtnWide" /></td>
							<td class="searchTitleArea"><label class="searchCondTitle">办件状态:</label>
							</td>
							<td class="searchCountArea"><s:radio name="s_bizstate"
									id="s_bizstate"
									cssStyle="color:#6b9bcf;background:none;border:none;vertical-align:middle;"
							...
							<td class="searchTitleArea"><label class="searchCondTitle">文件标题:</label>
							</td>
							<td class="searchCountArea"><input class="searchCondContent"
								type="text" name="s_income_Doc_Title"
								value="${s_income_Doc_Title }" /></td>


							<td class="fenggexian" style="padding-left: 0; padding-right: 0">
							</td>
							<td class="searchTitleArea"><label class="searchCondTitle">阅办文时间:</label>
							</td>
							<td class="searchCountArea"><input type="text"
								class="Wdate searchCondContent" id="s_begincomeDate"
								...
								placeholder="选择日期"> <label class="searchCondTitle">至</label>
								...
								onclick="WdatePicker({dateFmt:'yyyy-MM-dd '})"
								placeholder="选择日期">&nbsp;&nbsp; <input id="gaoji"
								type="button" value="高级" class="grayBtnWide"
								onclick="showgaoji()"> <input id="shouqi" type="button"
								value="收起" class="grayBtnWide" style="display: none;"
								onclick="toshouqi()"></td>
							<td class="searchTd" onclick="sub();"><div class="clickDiv"></div></td>
						<tr>
						<tr id="gaoji_more" style="display: none;">
							<td></td>
							<td class="searchTitleArea"><label class="searchCondTitle">阅办文号:</label>
							</td>
							<td class="searchCountArea"><input class="searchCondContent"
								style="width: 142px;" type="text" name="s_acceptArchiveNo"
								value="${s_acceptArchiveNo }" /></td>

							<td class="fenggexian" style="padding-left: 0; padding-right: 0">
							</td>
							<td class="searchTitleArea"><label class="searchCondTitle">拟发文字号:</label>
							</td>
							<td class="searchCountArea"><input class="searchCondContent"
								type="text" name="s_income_Doc_No" value="${s_income_Doc_No }" />
							</td>
							<td class="fenggexian" style="padding-left: 0; padding-right: 0">
							</td>
							<td class="searchTitleArea"><label class="searchCondTitle">发文单位:</label>
							</td>
							<td class="searchCountArea"><input class="searchCondContent"
								type="text" name="s_income_Dept_Name"
								value="${s_income_Dept_Name }" /></td>
						</tr>

						<c:if test="${not empty userRank}">
							<tr id="gaoji_more2" style="display: none;">
								<td></td>
								<td class="searchCondArea" colspan="2"><c:if
										test="${userRank=='TZ' }">
										<input type="checkbox" id="queryUnderUnit"
											name="queryUnderUnit" value="true"
								...
								<c:if test="${userRank=='TZ' }">
									<td class="fenggexian"
										style="padding-left: 0; padding-right: 0"></td>
									<td class="searchTitleArea"><label class="searchCondTitle">查询部门:</label>
									</td>
									<td class="searchCountArea"><select name="s_unitcode"
										class="searchCondContent">
						...
						<tr id="gaoji_more3" style="display: none;">
							<td></td>
							<td class="searchTitleArea"><label class="searchCondTitle">办理人员:</label>
							</td>
							<td class="searchCountArea"><input class="searchCondContent"
								style="width: 142px;" type="text" name="s_bizusernames"
								value="${s_bizusernames }" /></td>
	...
	<ec:table action="dispatchdoc/incomeDoc!list.do" items="incomeDocList"
		var="incomeDoc"
		imagePath="${pageContext.request.contextPath}/themes/css/images/table/*.gif"
		retrieveRowsCallback="limit">
		<ec:row>
			<ec:column property="none" title='序号' sortable="false"
				style="width:70px;text-align:centit;">
                  ${ROWCOUNT}
				</ec:column>
			...
			<ec:column property="incomeDocTitle" title="文件标题"
				...
				<c:choose>
					<c:when test="${fn:length(incomeDoc.incomeDocTitle) > 14}">
						<c:out value="${fn:substring(incomeDoc.incomeDocTitle, 0, 14)}..." />
					</c:when>
					<c:otherwise>
			...
			<ec:column property="incomeDeptName" title="发文单位"
				...
				<c:choose>
					<c:when test="${fn:length(incomeDoc.incomeDeptName) > 8}">
						<c:out value="${fn:substring(incomeDoc.incomeDeptName, 0,8)}..." />
					</c:when>
					<c:otherwise>
			...
			<ec:column property="incomeDate" title="阅办文时间"
			...
			<c:if test="${s_bizstate ne 'C' }">
				<ec:column property="nodename" title="办理步骤"
					...
					<c:choose>
						<c:when test="${fn:length(incomeDoc.nodename) >8}">
							<c:out value="${fn:substring(incomeDoc.nodename, 0,8)}..." />
						</c:when>
						<c:otherwise>
				...
				<ec:column property="bizusernames" title="办理人员"
					style="text-align:center" sortable="true" width="12%">
					<input type="hidden" value="${incomeDoc.bizusernames}" />
					<c:choose>
						<c:when test="${fn:length(incomeDoc.bizusernames) >12}">
							<c:out value="${fn:substring(incomeDoc.bizusernames, 0,12)}..." />
						</c:when>
						<c:otherwise>
			...
			<ec:column property="opt" title="${optlabel}" sortable="false"
				...
				<c:if test="${incomeDoc.bizstate ne 'F' }">
					<a class="check_email" href="javascript:void(0);"
						onclick="viewDetail('${incomeDoc.djId}',0)">查看</a>
				</c:if>
				<c:if test="${incomeDoc.bizstate eq 'F' }">
					<a class="bianji"
						href="${contextPath}/dispatchdoc/incomeDoc!registerDoOrReadEdit.do?djId=${incomeDoc.djId}">编辑</a>

					<a class="delete_email"
						href="<%=request.getContextPath()%>/dispatchdoc/incomeDoc!delete.do?djId=${incomeDoc.djId}">删除</a>
				</c:if>
				<c:if
					test="${cp:CHECKUSEROPTPOWER('SWGLSWCK', 'adminList') and incomeDoc.bizstate ne 'F' }">
					<a class="guanli"
						href="<%=request.getContextPath()%>/dispatchdoc/incomeDoc!listFlowInstNodes.do?flowInstId=${incomeDoc.flowInstId}">管理</a>
				</c:if>
			</ec:column>
		</ec:row>
	</ec:table>
...
//	 			$("#s_endTime").focus();
	...
	';
		...
		if ($("input[name=s_bizusernames]").val().trim() != ""
				&& $("input[name=s_bizusernames]").val() != null) {
			t = true;
		}
		return t;
	}
