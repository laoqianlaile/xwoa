	 ...
	 if(window.opener!=undefined){
  	...
  	try{
  	    if(window.screen.width>800)
  	    	objChild = window.open('${pageContext.request.contextPath}/sys/mainFrame!loginSuccess.do','XJOASYS','top=0,left=0,status=yes,menubar=no,scrollbars=yes,resizable=yes,location=no,width=2000,height=900');
  	    else
  	    	objChild = window.open('${pageContext.request.contextPath}/sys/mainFrame!loginSuccess.do','XJOASYS','top=0,left=0,status=yes,menubar=no,scrollbars=yes,resizable=yes,location=no');
  	    window.opener = null;
  	}catch(ex){
  		//alert("出于安全考虑，系统须在一个窗口中打开；如果您已经打开过系统，并且想重新进入，在“是否离开页面”询问框中点击“确定”。");
  		window.open('','_self');
  		window.close(); 
  		return;
  	}
  	 if (!reWork.test(String(objChild))) {
  		 alert("您打开了弹出窗口屏蔽功能，否则一些页面的功能可能不正常，请通过设置允许本站点弹出窗口。");
  	 }else{
  		 window.open('','_self');
  		 window.close(); 
  	 }
