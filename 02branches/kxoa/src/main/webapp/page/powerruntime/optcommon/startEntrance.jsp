...
<body>
    ...
    <c:if test="${itemtype eq 'SQ' }">
	 ...
	 <c:if test="${itemtype eq 'FW' }">
       <input type="button" name="glnbsq" class="btn btnWide" onclick="oaOpenGLFW();" value="关联已有拟发文" />
       <input type="button" name="fqqdfw" class="btn btnWide" onclick="openFQFW();" value="发起新拟发文" />
	 </c:if>
    ...
    <ec:table action="powerruntime/optApply!startEntrance.do" items="vbizBindInfos" var="suppower"
		...
		<ec:row>
		 ...
		 <ec:column property="transaffairname" title="办件名称" style="text-align:center" sortable="false">
			...
			<ec:column property="dispatchDocNo" title="拟发文号"	style="text-align:center" sortable="false"/>
