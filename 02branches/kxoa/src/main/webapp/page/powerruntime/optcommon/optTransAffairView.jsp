...
<div class="sider">
 ...
 <!-- 右侧导航 -->
	...
	<!--isDept为资产管理 厅机关展示标识, startDjid 事项关联查看也面展示标识  -->
	<c:if test="${isDept eq 'T' }">
	<input type="button" name="close" Class="btn" onclick="window.close();" value="关闭" /></c:if>
	<%-- <c:if test="${ not empty startDjid}">
	<input type="button" name="close" Class="btn" onclick="window.close();" value="关闭" /></c:if> --%>	
	<input type="hidden" value="${dashboard}">
	<c:if test="${empty startDjid and isDept ne 'T' and empty dashboard}">
	<input type="button" name="back" Class="btn" onclick="parent.navTab.reload();" value="返回" /></c:if>
