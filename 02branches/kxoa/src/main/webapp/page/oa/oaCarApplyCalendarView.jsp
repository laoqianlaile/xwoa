...
<c:set var="ajax" value="${pageContext.request.contextPath }/oa/oaCarApply!ajax.do?cardjid=${param['cardjid'] }" />
...
var calendar;
	...
	var options = {
		...
		callback : {
			...
			url='${contextPath}/oa/oaCarApply!addNew.do?' + 'planBegTime=' + Date.create(start).format('{yyyy}-{MM}-{dd} {HH}:{mm}:{ss}') + '&planEndTime=' + Date.create(end).format('{yyyy}-{MM}-{dd} {HH}:{mm}:{ss}')  + '&cardjid=${param.cardjid}';
			art.dialog
			.open(url,
					 {title: '车辆申请', width: 1050, height: 500});
// 			window.location = '${contextPath}/oa/oaCarApply!addNew.do?' + 'planBegTime=' + Date.create(start).format('{yyyy}-{MM}-{dd} {HH}:{mm}:{ss}') + '&planEndTime=' + Date.create(end).format('{yyyy}-{MM}-{dd} {HH}:{mm}:{ss}')  + '&cardjid=${param.cardjid}';
 			return;
			},
