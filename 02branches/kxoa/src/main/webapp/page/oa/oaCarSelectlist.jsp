...
<body>
					...
					<td><select name="s_carType"
						id="s_carType">
							...
							<c:forEach var="row" items="${oaCommonTypeList}">
								<option value="${row.no}"
									<c:if test="${s_carType eq row.no }">selected="selected"</c:if>>
	...
	<ec:table action="oa/oaCarinfo!selectList.do" items="objList" var="sa"
		...
		<ec:column property="oaDriverInfo.telephone" title="联系电话"
			...
			<ec:column property="responsibleDep" title="${tresponsibleDep}"
				style="text-align:center" >
				${cp:MAPVALUE('unitcode',sa.responsibleDep)}
				</ec:column> --%>
		...
		<%-- 	<c:set var="tresponsiblePersion">
			...
			<ec:column property="responsiblePersion"
				...
				${cp:MAPVALUE('usercode',sa.responsiblePersion)}
				</ec:column> --%>
...
// 		parentDocument.getElementById('djid').value = no;
		...
		//parentDocument.getElementById('spanbrand').innerHTML = document.getElementById('brand' + no).value;
	   ...
	   // parentDocument.getElementById('spanmodelType').innerHTML = document.getElementById('modelType' + no).value;
