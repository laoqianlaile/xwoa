...
function checkForm(){
	...
	if ("000692" == $("#wfcode").val() && "songqian" == $("nodeCode").val()) {
		if (!$("#dnTemplateId").val()) {
			alert("请保存送签说明书。");
			return false;
		}
...
function _getSelectedItemLabel(objSelect) {
   ...
   //alert(ideacode);
	    ...
	    //alert(ideacode);
	    if("qb_sh"==nodeCode){
			 if("FG"==ideacode || "B"==ideacode){
				if(_get("zbOrgRoleCodeTr")){_get("zbOrgRoleCodeTr").style.display = "block";}
				if(_get("xbArea")){_get("xbArea").style.display = "none";}
				//重置部门人员LABEL
				if("B"==ideacode){
					_get("zbTeamLabel").innerHTML='选择人员';
					_get("roleCode").value='qbsh';
				}else if("FG"==ideacode){
					_get("zbTeamLabel").innerHTML='${moduleParam.teamRoleName}';
					_get("roleCode").value='${moduleParam.teamRoleCode}';
				}
			 }else if("cs"==ideacode){				 
					if(_get("zbOrgRoleCodeTr")){_get("zbOrgRoleCodeTr").style.display = "none";}
					if(_get("xbArea")){_get("xbArea").style.display = "block";}				
			 }else{
				 if(_get("zbOrgRoleCodeTr")){_get("zbOrgRoleCodeTr").style.display = "none";}
					if(_get("xbArea")){_get("xbArea").style.display = "none";}
			 }
			 ...
			 if(_get("bjCodes")){_get('bjCodes').value='';}			
			 if(_get("xbOrgNames")){_get('xbOrgNames').value='';}
		 }
	    if("qb_ldqy"==nodeCode){//签报分管领导审批环节
			 if("F"==ideacode){
				if(_get("zbOrgRoleCodeTr")){_get("zbOrgRoleCodeTr").style.display = "block";}
				if(_get("xbArea")){_get("xbArea").style.display = "block";}
			 }else{
	    ...
	    if("qb_bgrcl"==nodeCode){//签报——签报办理人环节
			 if("T"==ideacode || "bm"==ideacode){
				if(_get("zbOrgRoleCodeTr")){_get("zbOrgRoleCodeTr").style.display = "none";}
				if(_get("xbArea")){_get("xbArea").style.display = "none";}
			 }else{
				 if(_get("zbOrgRoleCodeTr")){_get("zbOrgRoleCodeTr").style.display = "block";}
	    ...
	    if(_get("xbOrgNames")){_get('xbOrgNames').value='';}
	    if(_get("xbOrgCodes")){_get('xbOrgCodes').value='';}	    
	    adjustHeight();
}
...
<body class="sub-flow">
 ...
 <div class="group-title">
	...
	<s:form action="oATasksExcute" method="post" namespace="/oa"
		id="ioDocTasksForm" target="_parent" onsubmit="return checkForm();">
