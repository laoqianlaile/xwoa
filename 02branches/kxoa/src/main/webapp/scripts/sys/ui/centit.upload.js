$(function() {
	...
	$.myUpload = {
			...
			init: function() {
				...
				$(this).each(function() {
					...
					setOptionAttribute('needUploadDialog', $this, myOptions);
			...
			drawUploadDialog: function(settings) {
				var container = window.navTab ? navTab.getCurrentPanel() : $('body');
				
				if (settings.queueContainer) {
					container = $('#'+settings.queueContainer,container);
				}
				
				var dialog = window.DWZ ? $(window.DWZ.frag["PUBLIC_UPLOAD_CONTAINER"]).appendTo(container) : $(window.Centit.frag["PUBLIC_UPLOAD_CONTAINER"]).appendTo(container);
			...
			afterUploadSuccess: function(data) {
				...
				if (window.DWZ) {
					container.append('<tr id="file_'+filecode+'" > <td>'+filename+'</td> <td>'+filesize+'</td> <td><a href="'+window.DWZ.contextPath+'/app/fileinfo!deletefile.do?filecode='+filecode+'" class="delete" inputID="'+this.settings.inputID+'" filecode="'+filecode+'">删除</a></td><tr>');
				}
				else {
					container.append('<tr id="file_'+filecode+'" > <td>'+filename+'</td> <td>'+filesize+'</td> <td><a href="'+window.Config.contextPath+'/app/fileinfo!deletefile.do?filecode='+filecode+'" class="delete" inputID="'+this.settings.inputID+'" filecode="'+filecode+'">删除</a></td><tr>');
				}
			...
			onSelect: function(file) {
				...
				if (this.settings.needUploadDialog !== false) {
	...
	$.myUpload.options = {
		...
		// 必选设置
		swf: window.DWZ ? window.DWZ.contextPath+'/scripts/plugin/uploadify-v3.2.1/uploadify.swf' : window.Config.contextPath+'/scripts/plugin/uploadify-v3.2.1/uploadify.swf',
		uploader: window.DWZ ? window.DWZ.contextPath+'/app/fileinfofs!uploadfile.do' : window.Config.contextPath+'/app/fileinfo!uploadfile.do'
	};
});