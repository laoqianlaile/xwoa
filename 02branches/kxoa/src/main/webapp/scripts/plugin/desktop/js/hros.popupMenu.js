		...
		*/
		app : function(obj){
			...
			if(!TEMP.popupMenuApp){
				...
				TEMP.popupMenuApp = $('<div class="popup-menu app-menu" style="z-index:9990;display:none"><ul><li style="border-bottom:1px solid #F0F0F0"><a menu="open" href="javascript:;">打开应用</a></li><li><a menu="move" href="javascript:;">移动应用到<b class="arrow">»</b></a><div class="popup-menu" style="display:none"><ul><li><a menu="moveto" desk="1" href="javascript:;">桌面1</a></li><li><a menu="moveto" desk="2" href="javascript:;">桌面2</a></li><li><a menu="moveto" desk="3" href="javascript:;">桌面3</a></li><li><a menu="moveto" desk="4" href="javascript:;">桌面4</a></li><li><a menu="moveto" desk="5" href="javascript:;">桌面5</a></li></ul></div></li></ul></div>');
		...
		papp : function(obj){
			...
			$('.papp-menu a[menu="edit"]').off('click').on('click', function(){
				if(HROS.base.checkLogin()){
					$.dialog.open(HROS.CONFIG.contextPath + '/app/desktop!privateApp.do?id=' + obj.attr('appid'), {
						id : 'editdialog',
			...
			$('.papp-menu a[menu="del"]').off('click').on('click', function(){
				$.dialog({
					id : 'delpapp',
					title : '删除私人应用“' + obj.find('span').text() + '”',
					content : '删除应用将无法恢复',
					icon : 'warning',
					ok : function(){
						HROS.app.dataDeleteByAppid(obj.attr('appid'));
				...
				if(HROS.base.checkLogin()){
					$.dialog({
						...
						content : editFolderDialogTemp({
							...
							'contextPath' : HROS.CONFIG.contextPath
						}),
						ok : function(){
							var src = $('.folderSelector img').attr('src');
							src = src.substring(HROS.CONFIG.contextPath.length, src.length);
							if($('#folderName').val() != ''){
								$.ajax({
									type : 'POST',
									url : ajaxUrl,
									data : 'ac=updateFolder&name=' + $('#folderName').val() + '&icon=' + src + '&id=' + obj.attr('appid')
								}).done(function(responseText){
									HROS.app.get();
								});
							}else{
		...
		desk : function(){
			...
			if(!TEMP.popupMenuDesk){
				...
				$('.desk-menu a[menu="addfolder"]').on('click', function(){
					if(HROS.base.checkLogin()){
						$.dialog({
							...
							content : editFolderDialogTemp({
								'name' : '新建文件夹',
								'src' : HROS.CONFIG.contextPath + '/scripts/plugin/desktop/img/ui/folder_default.png',
								'contextPath' : HROS.CONFIG.contextPath
							}),
							ok : function(){
								var src = $('.folderSelector img').attr('src');
								...
								if($('#folderName').val() != ''){
									$.ajax({
										type : 'POST',
										url : ajaxUrl,
										data : 'ac=addFolder&name=' + $('#folderName').val() + '&icon=' + src + '&desk=' + HROS.CONFIG.desk
									}).done(function(responseText){
										HROS.app.get();
									});
								}else{
				...
				$('.desk-menu a[menu="addpapp"]').on('click', function(){
					if(HROS.base.checkLogin()){
						$.dialog.open(HROS.CONFIG.contextPath + '/app/desktop!privateApp.do?desk=' + HROS.CONFIG.desk, {
							id : 'editdialog',
							title : '新建私人应用',
							width : 600,
							height : 450
				...
				$('.desk-menu a[menu="themes"]').on('click', function(){
					if(HROS.base.checkLogin()){
						HROS.window.createTemp({
							appid : 'hoorayos-ztsz',
							title : '主题设置',
							url : HROS.CONFIG.contextPath + '/app/desktop!theme.do',
							width : 580,
							height : 520,
							isflash : false
						});
				...
				$('.desk-menu a[menu="setting"]').on('click', function(){
					if(HROS.base.checkLogin()){
						HROS.window.createTemp({
							appid : 'hoorayos-zmsz',
							title : '桌面设置',
							url : HROS.CONFIG.contextPath + '/app/desktop!desktopSetting.do',
							width : 750,
							height : 450,
							isflash : false
						});
			...
			$('.desk-menu a[menu="gotoManager"]').click(function(){
