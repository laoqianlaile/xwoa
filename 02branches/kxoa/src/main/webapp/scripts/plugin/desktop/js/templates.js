...
var appbtnTemp = template(
	'<li class="appbtn" id="<%=id%>" appid="<%=appid%>" realappid="<%=realappid%>" type="<%=type%>" top="<%=top%>" left="<%=left%>" style="top:<%=top%>px;left:<%=left%>px">'+
		'<div><img src="<%=imgsrc%>" title="<%=title%>"></div>'+
		'<span><%=title%></span>'+
	'</li>'
);
//桌面"添加应用"应用
	...
	'<div id="<%=id%>" appid="<%=appid%>" realappid="<%=realappid%>" type="<%=type%>" state="show" class="window-container window-current<% if(isflash){ %> window-container-flash<% } %>" style="<% if(isopenmax){ %>width:100%;height:100%;left:0;top:0;<% }else{ %>width:<%=width%>px;height:<%=height%>px;top:<%=top%>px;left:<%=left%>px;<% } %>z-index:<%=zIndex%>" ismax="<% if(isopenmax){ %>1<% }else{ %>0<% } %>">'+
		'<div style="height:100%">'+
			...
			'<div class="control-handle">' +
				'<a class="btn disabled goback" title="后退">' + 
					'<i class="icon icon8"></i>' +
				'</a>' +
//				'<a class="btn gonext" title="前进">' + 
//					'<i class="icon icon9"></i>' +
//				'</a>' +
				'<a class="indicator indicator-manage" title="全局视图"></a>' +
				'<a class="btn refresh" title="刷新">' + 
					'<i class="icon icon158"></i>' +
				'</a>' +
			'</div>' +
			...
			'<div class="window-frame">'+
				...
				'<iframe id="<%=id%>_iframe" frameborder="0" src="<%=url%>" onload="$.history.save(this)"></iframe>'+
			'</div>'+
...
//全局文件夹预览
var folderViewOverTemp = template(
	'<div id="<%=id%>" appid="<%=appid%>" realappid="<%=realappid%>" class="quick_view_container overview" style="top:<%=top%>px;left:<%=left%>px">'+
		'<div class="perfect_nine_box">'+
			'<div class="perfect_nine_t">'+
				'<div class="perfect_nine_t_m"></div>'+
			'</div>'+
...
var editFolderDialogTemp = template(
	'<div id="addfolder">'+
		...
		'<div class="fcDropdown">'+
			'<a class="fcDropdown_item" title="默认"><img class="fcDropdown_img" src="<%=contextPath%>/scripts/plugin/desktop/img/ui/folder_default.png"></a>'+
			'<a class="fcDropdown_item" title="文本"><img class="fcDropdown_img" src="<%=contextPath%>/scripts/plugin/desktop/img/ui/folder_doc.png"></a>'+
			'<a class="fcDropdown_item" title="游戏"><img class="fcDropdown_img" src="<%=contextPath%>/scripts/plugin/desktop/img/ui/folder_game.png"></a>'+
			'<a class="fcDropdown_item" title="生活"><img class="fcDropdown_img" src="<%=contextPath%>/scripts/plugin/desktop/img/ui/folder_life.png"></a>'+
			'<a class="fcDropdown_item" title="音乐"><img class="fcDropdown_img" src="<%=contextPath%>/scripts/plugin/desktop/img/ui/folder_music.png"></a>'+
			'<a class="fcDropdown_item" title="工具"><img class="fcDropdown_img" src="<%=contextPath%>/scripts/plugin/desktop/img/ui/folder_tool.png"></a>'+
			'<a class="fcDropdown_item" title="视频"><img class="fcDropdown_img" src="<%=contextPath%>/scripts/plugin/desktop/img/ui/folder_video.png"></a>'+
		'</div>'+
	'</div>'
);
//应用评分
	...
	'<div id="help">'+
		...
		'<div id="step1" class="step" step="1" style="position:relative;left:50%;top:50%;margin-left:-160px;margin-top:-60px;width:280px;height:100px">'+
			'<p style="text-align:center">'+
				'<span class="h2">欢迎使用 南大先腾 OA系统</span>'+
