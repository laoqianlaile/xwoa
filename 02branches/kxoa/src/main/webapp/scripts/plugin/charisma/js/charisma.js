$(document).ready(function(){
	...
	// 设置弹出框样式AIR
	$._messengerDefaults = {
		extraClasses: 'messenger-fixed messenger-theme-bootstrap messenger-on-top'
	};
	
	jQuery.validator.addMethod("ltDate", function(value, element, param) {
		var other = $(param);
		if (!value || !other.val()) return this.optional(element);
		
		return this.optional(element) || (value <= other.val());
	}, $.validator.format("起始时间不能大于结束时间!"));
	...
	jQuery.validator.addMethod("gtDate", function(value, element, param) {
		...
		return this.optional(element) || (value >= other.val());
	}, $.validator.format("结束时间不能小于起始时间!"));
	
	
	jQuery.validator.addMethod("idcard", function(value, element) {
		if (!value) return this.optional(element);
		
		return this.optional(element) || /^[\d]{6}((19[\d]{2})|(200[0-8]))((0[1-9])|(1[0-2]))((0[1-9])|([12][\d])|(3[01]))[\d]{3}[0-9xX]$/.test(value);
	...
	jQuery.validator.addMethod("telephone", function(value, element) {
		...
		/^(^(1[3|4|5|8][0-9])\d{8}$)|^((\d{6,8})|(\d{4}|\d{3})-(\d{6,8})|(\d{4}|\d{3})-(\d{6,8})-(\d{4}|\d{3}|\d{2}|\d{1})|(\d{6,8})-(\d{4}|\d{3}|\d{2}|\d{1}))$/.test(value);
	...
	jQuery.validator.addMethod("loginname", function(value, element) {
		...
		return this.optional(element) || 
		/^\w+$/.test(value);
	}, $.validator.format("登录名只可以是数字、字母或划线组成!"));	
...
//		
////			"sDom": "<'row'<'span12 center'p><'span6'i><'span6 pagination-right'l>>t<'row'<'span6'i><'span6 pagination-right'l><'span12 center'p>>",
//			
//			"sDom": "t",
...
//			"bAutoWidth":false,
//			"oLanguage": {
//				"sInfo":"显示 第_START_ 到 第_END_ 条记录，共 _TOTAL_ 条",
