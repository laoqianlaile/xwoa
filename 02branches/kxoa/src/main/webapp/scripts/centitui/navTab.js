...
var navTab = {
	...
	_op: {id:"navTab", stTabBox:".navTab-tab", stPanelBox:".navTab-panel", mainTabId:"main", close$:"a.close", prevClass:"tabsLeft", nextClass:"tabsRight", stMore:".tabsMore", stMoreLi:".tabsMoreList ul"},
	...
	init: function(options){
		...
		this._moreBut.click(function(){
			$this._moreBox.parent().show();
			return false;
		});
		$("#closeAll").click(function(){$this.closeAllTab()});
		$("#home-page").click(function(e){
			e.preventDefault();
			$this._switchTab(0);
			
		});
		$(document).click(function(){$this._moreBox.parent().hide()});
			...
			$(this).unbind("click").click(function(event){
				...
				if(iTabIndex == 0){
					...
					if($("#sidebar").is(":visible")){
						$("#sidebar").hide();
						$("#container").css({left:"5px",width:$("#container").width()+170});	    			  
					}
					if($("#sidebar_s").is(":visible")){
						$("#sidebar_s").hide();
						$("#container").css({left:"5px",width:$("#container").width()+24});
					}
				}else{					
										
					if(!$("#sidebar").is(":visible") && !$("#sidebar_s").is(":visible")){
			    	    $("#sidebar").show();
			    	    $("#sidebar").css({left:"5px"});
			    	    
			    	    $("#container").css({left:"175px",width:$("#container").width()-170});	
			    	    $('#toggleCollapseDiv').click();
	...
	_contextmenu:function($obj){ // navTab右键菜单
		...
		$obj.contextMenu('navTabCM', {
			...
			ctrSub:function(t,m){
				...
				if(t.attr("tabid") != "main"){
					mCur.removeClass("disabled");
				}
				if( t.attr("tabid") == undefined ){
					mReload.addClass("disabled");
	...
	_scrollTab: function(iLeft, isNext){
		...
		//$("li:first",this._tabBox).addClass("li-first").css({left:Math.abs(iLeft)-2+"px"});
	},
	_scrollCurrent: function(){ // auto scroll current tab
		var iW = this._tabsW(this._getTabs());
		//alert(iW);IE 99   other 110
	...
	_switchTab: function(iTabIndex){	
		/*if(iTabIndex==0){
			...
			if(!$("#sidebar").is(":visible")&&!$("#sidebar_s").is(":visible")){
				$("#sidebar_s").show().css({left:"5px"});
				$("#container").css({left:"29px",width:$("#container").width()-24});
			}
		}*/	
	...
	_reload: function($tab, flag){
		...
		if (flag && url) {
			...
			if ($tab.hasClass("external")){
				navTab.openExternal(url,$tab.attr("tabid"),$panel);
			}else {
	...
	reload: function(url, options){
		...
		if ($panel){
			...
			if (url) {
				if ($tab.hasClass("external")) {
					navTab.openExternal(url,op.navTabId,$panel);
				} else {
					$panel.ajaxUrl({
	...
	checkTimeout:function(){
		var json = CentitUI.jsonEval(this.getCurrentPanel().html());
		if (json && json.statusCode == CentitUI.statusCode.timeout) this.closeCurrentTab();
	},
	openExternal:function(url,id,$panel){
		var ih = navTab._panelBox.height();
		var iframe = $panel.find('iframe')[0];
		
		//if (document.frames && iframe) {
			//iframe = document.frames[id];
				
//			if (url == iframe.location.href) {
...
//			}
		...
		$panel.html(CentitUI.frag["externalFrag"].replaceAll("{url}", url).replaceAll("{id}",id).replaceAll("{height}", ih-7+"px"));
	},
	/**
	 * 
	 * @param {Object} tabid
	...
	openTab: function(tabid, url, options){ //if found tabid replace tab, else create a new tab.
		...
		if (iOpenIndex >= 0){
			...
			var tabFrag = '<li tabid="#tabid#"><a title=""><span>#title#</span></a><a class="close">close</a></li>';
			this._tabBox.append(tabFrag.replace("#tabid#", tabid).replaceAll("#title#", op.title));
			this._panelBox.append('<div class="page unitBox" ma-bootstrap></div>');
			this._moreBox.append('<li><a title="#title#">#title#</a></li>'.replaceAll("#title#", op.title));
