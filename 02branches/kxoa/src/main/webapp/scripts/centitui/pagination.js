	...
	$.fn.extend({
		pagination: function(opts){
			...
			return this.each(function(){
				...
				$this.html(CentitUI.frag["pagination"].replaceAll("#pageNumFrag#", pageNumFrag).replaceAll("#currentPage#", pc.getCurrentPage())).find("li").hoverClass();
				...
				$this.find(setting.jumpto$).each(function(){
					...
					$button.click(function(event){
						...
						if (pageNum && pageNum.isPositiveInteger()) {
							CentitUIPageBreak({targetType:pc.targetType(), rel:pc.rel(), data: {pageNum:pageNum}});
						}
					});
					$inputBox.keyup(function(event){
						if (event.keyCode == CentitUI.keyCode.ENTER) $button.click();
					});
				});
			});
			...
			function _bindEvent($target, pageNum, targetType, rel){
				$target.bind("click", {pageNum:pageNum}, function(event){
					CentitUIPageBreak({targetType:targetType, rel:rel, data:{pageNum:event.data.pageNum}});
		...
		orderBy: function(options){
			...
			return this.each(function(){
				var $this = $(this).css({cursor:"pointer"}).click(function(){
					...
					CentitUIPageBreak({targetType:op.targetType, rel:op.rel, data:{orderField: orderField, orderDirection: orderDirection}});
				});
