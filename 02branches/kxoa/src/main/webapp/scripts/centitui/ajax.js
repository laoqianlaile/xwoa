...
function validateCallback(form, callback) {
	...
	$.ajax({
		...
		dataType:"json",
		cache: false,
		success: callback || CentitUI.ajaxDone,
		error: CentitUI.ajaxError
	});
	return false;
}
...
/**
 * 普通ajax表单提交
 * @param {Object} form  button
 * @param {Object} callback
 */
function methodCallback(form, button,callback) {
	var $form = $(form);
	if (!$form.valid()) {
		return false;
	}
	var $button=$(button);
	alert($form.attr("action")+"?"+$button.attr("name"));
	$.ajax({
		type: form.method || 'POST',
		url:$form.attr("action")+"?"+$button.attr("name"),
		data:$form.serializeArray(),
		dataType:"json",
		cache: false,
		success: callback || CentitUI.ajaxDone,
...
function iframeCallback(form, callback){
	...
	_iframeResponse($iframe[0], callback || CentitUI.ajaxDone);
}
...
/**
 * navTabAjaxDone是CentitUI框架中预定义的表单提交回调函数．
 * 服务器转回navTabId可以把那个navTab标记为reloadFlag=1, 下次切换到那个navTab时会重新载入内容. 
 * callbackType如果是closeCurrent就会关闭当前tab
 * 只有callbackType="forward"时需要forwardUrl值
 * navTabAjaxDone这个回调函数基本可以通用了，如果还有特殊需要也可以自定义回调函数.
 * 如果表单提交只提示操作是否成功, 就可以不指定回调函数. 框架会默认调用CentitUI.ajaxDone()
 * <form action="/user.do?method=save" onsubmit="return validateCallback(this, navTabAjaxDone)">
 * 
 * form提交后返回json数据结构statusCode=CentitUI.statusCode.ok表示操作成功, 做页面跳转等操作. statusCode=CentitUI.statusCode.error表示操作失败, 提示错误原因. 
 * statusCode=CentitUI.statusCode.timeout表示session超时，下次点击时跳转到CentitUI.loginUrl
...
function navTabAjaxDone(json){
	CentitUI.ajaxDone(json);
	if (json.statusCode == CentitUI.statusCode.ok){
...
/**
 * dialog上的表单提交回调函数
 * 服务器转回navTabId，可以重新载入指定的navTab. statusCode=CentitUI.statusCode.ok表示操作成功, 自动关闭当前dialog
 * 
...
function dialogAjaxDone(json){
	...
	if (json.statusCode == CentitUI.statusCode.ok){
		...
		//周杰 2012.2.8  newWin属性刷新当前窗口
		else if(json.newWin){
			window.open(json.forwardUrl,"_self");
		}
...
function navTabSearch(form, navTabId){
	var $form = $(form);
	if (form[CentitUI.pageInfo.pageNum]) form[CentitUI.pageInfo.pageNum].value = 1;
...
function CentitUISearch(form, targetType){
...
function _getPagerForm($parent, args) {
	...
	if (form) {
		if (args["pageNum"]) form[CentitUI.pageInfo.pageNum].value = args["pageNum"];
		if (args["numPerPage"]) form[CentitUI.pageInfo.numPerPage].value = args["numPerPage"];
		if (args["orderField"]) form[CentitUI.pageInfo.orderField].value = args["orderField"];
		if (args["orderDirection"] && form[CentitUI.pageInfo.orderDirection]) form[CentitUI.pageInfo.orderDirection].value = args["orderDirection"];
	}
 ...
 * callback: 加载完成回调函数
 */
function CentitUIPageBreak(options){
...
function navTabPageBreak(args, rel){
	CentitUIPageBreak({targetType:"navTab", rel:rel, data:args});
}
/**
 * 处理dialog中的分页和排序
 * 参数同 navTabPageBreak 
...
function dialogPageBreak(args, rel){
	CentitUIPageBreak({targetType:"dialog", rel:rel, data:args});
}
...
function uploadifyComplete(event, queueId, fileObj, response, data){
	CentitUI.ajaxDone(CentitUI.jsonEval(response));
}
