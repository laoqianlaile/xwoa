...
function RAP(nick, element)

{
	rtx_addPresence(nick, element);
}

function rtx_addPresence(nick, element)
{
	var src = element || event.srcElement;
	var id;
...
function rtx_nameControlLoadTimer()
{
	if(!rtx_isNameControlLoadError)
	{
		...
		if(obj && typeof(obj.PresenceEnabled) != "undefined")
		{
			...
			{
//				alert("rtx_nameControlLoadTimer")
...
function rtx_updateImage(id, img)
{
	...
	if(obj && typeof(img) == "string")
	{
		...
		var index;
		var newImg;
//		alert("oldImg"+oldImg +"img"+img)
		if(!obj.imageBaseURL)
		{
			index = oldImg.lastIndexOf("/");
			obj.imageBaseURL = oldImg.slice(0, index + 1);
//			alert("index"+index +"obj.imageBaseURL"+obj.imageBaseURL)
		}

		index = img.lastIndexOf("\\");
	
        newImg = img.slice(index+1);
	    newImg=obj.imageBaseURL+newImg;
//		alert(newImg)
		if(oldImg != newImg)
		{
