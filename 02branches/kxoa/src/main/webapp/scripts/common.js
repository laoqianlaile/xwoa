...
function genHandleComments(obj) {	
	...
	var auditUserRoleCode = "", auditUserNames = "", zbUserRoleCode = "", zbUserNames = "";
	...
	if (obj["xbOrgNames"]) { // 分办机构
		xbOrgNames = obj["xbOrgNames"].val();
		xbOrgRoleCode = obj["xbOrgRoleCode"].val();
	}
	if (obj["engineUserNames"]) { // 指定人员
		engineUserNames = obj["engineUserNames"].val();
		engineRoleCode = obj["engineRoleCode"].val();
	}
	if (obj["auditUserNames"]) { // 指定人员
		auditUserNames = obj["auditUserNames"].val();
		auditUserRoleCode = obj["auditUserRoleCode"].val();
	}
	if (obj["zbUserNames"]) { // 指定人员
...
if (zbOrgName_ || xbOrgNames_ || ldzdxName_) {
	...
	if (bjUserNames || zbOrgName || xbOrgNames || engineUserNames || auditUserNames || zbUserNames) {
		comments.push("请");
		if (bjUserNames || engineUserNames || auditUserNames || zbUserNames) {
			...
			if (auditUserNames) {
				...
				comments.push(auditUserNames.split(",").join("、") + "同志阅处");
			}
 ...
 * @param h
 */
DialogUtil.prototype.open = function(title,url,w,h,okcall,cancelcall,closecall){
