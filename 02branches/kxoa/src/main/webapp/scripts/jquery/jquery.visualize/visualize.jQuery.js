			...
			var tableData = {
				dataGroups: function(){
					...
					if(o.parseDirection == 'x'){
						self.find('tr:gt(0):visible').each(function(i){
							dataGroups[i] = {};
							dataGroups[i].points = [];
...
//							$(this).find('td').each(function(){
							$(this).find('td:gt(0):visible').each(function(){
...
//						var cols = self.find('tr:eq(1) td').size();
						var cols = self.find('tr:eq(1) td').size()-1;
						for(var i=0; i<cols; i++){
							dataGroups[i] = {};
							dataGroups[i].points = [];
							dataGroups[i].color = colors[i];
							if(textColors[i]){ dataGroups[i].textColor = textColors[i]; }
							self.find('tr:gt(0):visible').each(function(){
//								modify by zk
//								dataGroups[i].points.push( $(this).find('td').eq(i).text()*1 );
								dataGroups[i].points.push( $(this).find('td:visible').eq(i+1).text()*1 );
							});
						};
					}
					return dataGroups;
...
//						self.find('tr:eq(0) th').each(function(){
						self.find('tr:eq(0) th:gt(0):visible').each(function(){
...
//						self.find('tr:gt(0) th').each(function(){
						self.find('tr:gt(0):visible td:first-child:visible').each(function(){
							xLabels.push($(this).html());
						});
					}
					return xLabels;
...
//			var selector = (o.parseDirection == 'x') ? 'tr:gt(0) th' : 'tr:eq(0) th' ;
			var selector = (o.parseDirection == 'x') ? 'tr:gt(0):visible td:first-child' : 'tr:eq(0) th:gt(0):visible' ;
